 ...
 */
class availability_frontend extends \core_availability\frontend {

    /**
     * Includes JavaScript for the main system and only group plugins.
     *
    ...
    public static function include_all_javascript($course, \cm_info $cm = null,
        ...
        // into a single call (the main init function will call init for each plugin).
        $modules = [
        ...
        // Filter only groups and gruping.
        $allenabled = $pluginmanager->get_enabled_plugins('availability');
        $enabled = [];
        if (isset($allenabled['group'])) {
            $enabled['group'] = $allenabled['group'];
        }
        if (isset($allenabled['grouping'])) {
            $enabled['grouping'] = $allenabled['grouping'];
        }
        // End filter only groups and gruping.
