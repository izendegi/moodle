...
// Support both activity and course-level downloads.
...
if ($cmid) {
    ...
    $pageurl = new moodle_url(
        '/report/embedquestion/responsedownload.php',
    ...
    $pageurl = new moodle_url(
        '/report/embedquestion/responsedownload.php',
        ['courseid' => $courseid, 'file' => $file, 'size' => $size]
    );
    $reporturl = new moodle_url('/report/embedquestion/index.php', ['courseid' => $courseid]);
} else {
    throw new moodle_exception('invalidrequest', 'report_embedquestion');
}

if (
    !has_capability('report/embedquestion:downloadanyattempt', $context) &&
    !has_capability('report/embedquestion:downloadmyattempt', $context)
) {
...
if ($cmid) {
    $params = ['cmid' => $cmid, 'file' => $file, 'size' => $size];
} else {
    $params = ['courseid' => $courseid, 'file' => $file, 'size' => $size];
}
$navtitle = get_string('pluginname', 'report_embedquestion');
$pagetitle = get_string('downloadresponse_title', 'report_embedquestion');
$PAGE->navbar->add($navtitle, $reporturl);
...
echo $renderer->render_responses_download_links($params);
