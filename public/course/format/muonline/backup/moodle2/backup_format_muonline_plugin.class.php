...
/**
 * Specialised backup for format_muonline
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_format_muonline_plugin extends backup_format_plugin {
    /**
    ...
    protected function define_section_plugin_structure() {
        ...
        $plugin = $this->get_plugin_element(null, $this->get_format_condition(), 'muonline');

        // Define each element separated.
        // Adds tag <plugin_format_muonline_section> within section.xml files of backup file with filename / icon name.
        // The "id" field below is the mdl_format_muonline_tile_options.id in the old course.
        // It will appear in section.xml as <plugin_format_muonline_section sectionid="{sectionid}">
        // This is then used in the restore section as $data['sectionid'] in process_muonline_section();
        // The optiontype and optionvalue fields will be included in each <plugin_format_muonline_section> tag.
        $sectionoptions = new backup_nested_element(
            $this->get_recommended_name(), // Expect 'plugin_format_muonline_section'.
            ['sectionid'],
            ['optiontype', 'optionvalue', 'muonlineversion']
        );

        // Define sources.
        $const1 = \format_muonline\local\format_option::OPTION_SECTION_PHOTO;
        $const2 = \format_muonline\local\format_option::OPTION_SECTION_ICON;

        // Include muonline version so we can use it on restore.
        $muonlineversion  = filter_var(get_config('format_muonline', 'version') ?? 0, FILTER_SANITIZE_NUMBER_INT);

        $sectionoptions->set_source_sql(
            "SELECT elementid as sectionid, optiontype, optionvalue, $muonlineversion AS muonlineversion
                FROM {format_muonline_tile_options}
    ...
    private function pre_backup_steps() {
        ...
        // Starting muonline plugin version 4.3, the course_format_options table is not used for tile photos/icons.
        ...
        // There is an observer \format_muonline\observer\course_backup_created() which reverses this once backup is complete.
        \format_muonline\local\format_option::set_legacy_format_options($courseid);
    }
    ...
    private function pre_step_fail_if_course_includes_excess_sections(int $courseid) {
        ...
        $maxsectionsconfig = \format_muonline\local\course_section_manager::get_max_sections();
        ...
        if ($isadmin) {
            $admintoolsurl = \format_muonline\local\course_section_manager::get_list_problem_courses_url();
        ...
        if (\format_muonline\local\course_section_manager::course_section_count_exceeds($courseid, $maxallowed * 5)) {
            ...
            \core\notification::error(get_string('restoretoomanysections', 'format_muonline', $maxallowed) . $admintoolsbutton);
            throw new moodle_exception('backupfailed', 'format_muonline', $admintoolsurl);
        }
        ...
        foreach ($sections as $section) {
            ...
            if ($included) {
                ...
                if ($totalincluded > $maxallowed * 5) {
                    ...
                    // Check can probably be removed soon.
                    \core\notification::error(
                        get_string('restoretoomanysections', 'format_muonline', $maxsectionsconfig) . $admintoolsbutton
                    );
                    throw new moodle_exception('backupfailed', 'format_muonline', $admintoolsurl);
                }
            }
