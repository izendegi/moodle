...
/**
 * Specialised restore for format_muonline (based on the equivalent for format_topics
 *
...
/**
 * Specialised restore for format_muonline
 *
 * Processes 'numsections' from the old backup files and hides sections that used to be "orphaned".
 * Also handles restoring tile background image files from the backup archive to the muonline.
 *
 * @package   format_muonline
 * @category  backup
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_format_muonline_plugin extends restore_format_plugin {
    /** @var int */
    ...
    public function define_section_plugin_structure() {
        ...
        // This defines 'process_muonline_section' below.
        return [new restore_path_element('muonline_section', $this->get_pathfor('/'))];
    }
     ...
     * @throws moodle_exception
     */
    public function process_muonline_section($data) {
        global $DB;
        if ($this->is_muonline_course()) {
            $this->set_format_opt(
            ...
            if (!$oldsectionid) {
                // This should rarely happen - only if backup was prepared with early Muonline 4.3 beta.
    ...
    public function after_restore_section() {
        ...
        if ($this->is_muonline_course()) {
            ...
            if (isset($data['path']) && $data['path'] = '/section' && isset($data['tags']['id'])) {
                ...
                if ($newsectionid) {
                    // This won't be needed if the backup was done with muonline plugin version higher than 2024021400.
                    // Higher than that version, it's mapped in $this->set_mapping('itemid ... in process_muonline_section().
        ...
        if ($this->is_muonline_course()) {
            ...
            if (!$needsunmappedfilesadded && version_compare($backuprelease, '4.3', '<=')) {
                ...
                $hasmappedfiles = $DB->record_exists_sql(
                    "SELECT fo.id
                        FROM {format_muonline_tile_options} fo
                        JOIN {files} f ON f.itemid = fo.elementid AND f.contextid = :coursecontextid
                        AND f.component = 'format_muonline' AND f.filearea = 'tilephoto' AND f.filepath = '/tilephoto/'
            ...
            $currentfilterbarsetting = $DB->get_record(
                'course_format_options',
                ['name' => 'displayfilterbar', 'format' => 'muonline', 'courseid' => $newcourseid]
            );
            ...
            if ($isoutcomesfilter) {
                ...
                $DB->delete_records(
                    'course_format_options',
                    ['name' => 'tileoutcomeid', 'format' => 'muonline', 'courseid' => $newcourseid]
                );
            }
            ...
            $DB->set_field(
                ...
                'name',
                'defaulttileicon',
                ['format' => 'muonline', 'name' => 'defaulttiletopleftdisplay', 'courseid' => $newcourseid]
            );
            $DB->set_field(
                'course_format_options',
                'name',
                'tileicon',
                ['format' => 'muonline', 'name' => 'tiletopleftthistile', 'courseid' => $newcourseid]
            );
            ...
            $DB->delete_records_select(
                'course_format_options',
                "format  = 'muonline' AND name = 'tileicon' AND value = 'course default' AND courseid = :courseid",
                ["courseid" => $newcourseid]
            );
            ...
            $fs = get_file_storage();
            $fs->delete_area_files(context_course::instance($newcourseid)->id, 'format_muonline', 'temptilephoto');
        }
    }
     ...
     * @param int $contextid
     * @param int $olditemid the old itemid (is used to contain section id for section muonline)
     * @param int $newitemid the new itemid (is used to contain section id for section muonline)
     * @return void
     * @throws dml_exception
     * @throws file_exception
    ...
    private static function update_file_records_sections($newcourseid, $contextid, $olditemid, $newitemid) {
        ...
        $fileids = $DB->get_fieldset_select(
            'files',
            'id',
            "contextid = :coursecontextid AND component = 'format_muonline'
        ...
        if (!empty($fileids)) {
            // This won't be executed if the backup was done with muonline plugin version higher than 2024021400.
            ...
            foreach ($fileids as $fileid) {
                ...
                if ($oldfile && $oldfile->get_itemid() != $newitemid) {
                    ...
                    \format_muonline\local\tile_photo::delete_files_from_ids($newcourseid, $newitemid);
    ...
    /**
     * Process an item for format_muonline_tile_options table.
     * @param int $courseid
     * @param int $sectionid
    ...
    private function set_format_opt(int $courseid, int $sectionid, int $optiontype, string $optionvalue) {
        try {
            ...
            throw new moodle_exception(
                ...
                'format_muonline',
                '',
                'Could not insert photo. Database table format_muonline_tile_options is not ready.' .
    ...
    /**
     * Issue 45.
     * If incompatible Moodle 3.7 version of Muonline plugin was used in Moodle 3.9, incorrectly numbered sections may exist.
    ...
    private function fail_if_course_includes_excess_sections() {
        $backupinfo = $this->step->get_task()->get_info();
        $maxallowed = \format_muonline\local\course_section_manager::get_max_sections();
        ...
        foreach ($backupinfo->sections as $section) {
            ...
            if ($included) {
                ...
                if ($totalincluded > ($maxallowed + 1) * 5) {
                    ...
                    \core\notification::error(get_string('restoretoomanysections', 'format_muonline', $maxallowed));
                    throw new moodle_exception('restoretoomanysections', 'format_muonline', '', $maxallowed);
                }
            }
        }
    }
    ...
    private function check_destination_course_section_count() {
        ...
        // Check can probably be removed soon.
        if (\format_muonline\local\course_section_manager::course_section_count_exceeds($courseid, $maxallowed * 5)) {
            $a = new stdClass();
            $a->sectionnum = \format_muonline\local\course_section_manager::get_max_non_subsection_number($courseid);
            $a->maxallowed = $maxallowed;
            \core\notification::error(get_string('restoreincorrectsections', 'format_muonline', $a));
            throw new moodle_exception('restorefailed', 'format_muonline', '', $a);
        }
    ...
    /**
     * Most of the public methods in this class are called on restore of all courses, not just Muonline format.
     * In some cases we check that the format we are restoring is Muonline and only act if it is.
     * @return bool
     */
    private function is_muonline_course(): bool {
        $backupinfo = $this->step->get_task()->get_info();
        return isset($backupinfo->original_course_format) && $backupinfo->original_course_format == 'muonline';
    }
}
