...
/**
 * This file contains main class for the course format Muonline
 *
 * @since     Moodle 2.7
...
/**
 * Main class for the course format Muonline
 *
 * @since     Moodle 2.7
 * @package   format_muonline
 * @copyright 2016 David Watson {@link http://evolutioncode.uk}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_muonline extends core_courseformat\base {
    /**
    ...
    public function uses_indentation(): bool {
        return isset($this->course->courseusesubmuonline) && !$this->course->courseusesubmuonline;
    }
    ...
    public function uses_course_index() {
        return get_config('format_muonline', 'usecourseindex');
    }
    ...
    public function get_default_section_name($section) {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_muonline');
        } else {
    ...
    public function edit_form_validation($data, $files, $errors) {
        ...
        if (!$data['enablecompletion'] && $data['courseshowtileprogress']) {
            $reterrors['courseshowtileprogress'] = get_string('courseshowtileprogress_error', 'format_muonline');
        }
        if (
            ($data['displayfilterbar'] == \format_muonline\local\format_option::FILTER_OUTCOMES_ONLY
                || $data['displayfilterbar'] == \format_muonline\local\format_option::FILTER_OUTCOMES_AND_NUMBERS)
            && empty(\format_muonline\local\filters::get_course_outcomes($courseid))
        ) {
            $outcomeslink = html_writer::link(
                new moodle_url('/grade/edit/outcome/course.php', ['id' => $courseid]),
                new lang_string('outcomes', 'format_muonline')
            );
            $reterrors['displayfilterbar'] = get_string('displayfilterbar_error', 'format_muonline') . ' ' . $outcomeslink;
        }
        return $reterrors;
    }
    ...
    public function extend_course_navigation($navigation, navigation_node $node) {
        ...
        if ($PAGE->user_is_editing() && has_capability('moodle/site:config', context_system::instance())) {
            $settingsnode = navigation_node::create(
                get_string('resetalltiles', 'format_muonline'),
                new moodle_url('/course/format/muonline/editor/reset.php', ['courseid' => $course->id]),
    ...
    // MDL-79986 introduced new /course/section.php page whereas muonline had used course/view.php?section=xx.
    ...
    public function course_header() {
        ...
        if (\format_muonline\local\util::using_js_nav()) {
            // No output needed in this case.
            return null;
        }

        $renderer = $PAGE->get_renderer('format_muonline');
        ...
        return new \format_muonline\output\course_header_content($course, false, 0, $renderer);
    }
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            $courseformatoptions = [
                ...
                'basecolour' => [
                    'default' => get_config('format_muonline', 'tilecolour1'),
                    'type' => PARAM_TEXT,
                ],
                'courseshowtileprogress' => ['default' => 0, 'type' => PARAM_INT],
                'displayfilterbar' => ['default' => 0, 'type' => PARAM_INT],
            ];
            if ((get_config('format_muonline', 'followthemecolour'))) {
                unset($courseformatoptions['basecolour']);
            }
        }

        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            $muonlinepalette = \format_muonline\local\util::get_muonline_palette();
            $tileicons = (new \format_muonline\local\icon_set())->available_tile_icons($this->get_courseid());
            ...
            $label = get_string('defaulttileicon', 'format_muonline');
            $courseformatoptionsedit['defaulttileicon'] = [
                'label' => $label,
                'element_type' => 'select',
                'element_attributes' => [$tileicons],
                'help' => 'defaulttileicon',
                'help_component' => 'format_muonline',
            ];
            if (!(get_config('format_muonline', 'followthemecolour'))) {
                $courseformatoptionsedit['basecolour'] = [
                    'label' => new lang_string('basecolour', 'format_muonline'),
                    'element_type' => 'select',
                    'element_attributes' => [$muonlinepalette],
                    'help' => 'basecolour',
                    'help_component' => 'format_muonline',
                ];
            }
            $attributes = [
                \format_muonline\local\format_option::FILTER_NONE => new lang_string('hide', 'format_muonline'),
                \format_muonline\local\format_option::FILTER_NUMBERS_ONLY => new lang_string('filternumbers', 'format_muonline'),
            ];
            $outcomeslink = '(' . new lang_string('outcomesunavailable', 'format_muonline') . ')';
            global $CFG;
            if (!empty($CFG->enableoutcomes)) {
                ...
                $attributes[\format_muonline\local\format_option::FILTER_OUTCOMES_ONLY]
                    = new lang_string('filteroutcomes', 'format_muonline');
                $attributes[\format_muonline\local\format_option::FILTER_OUTCOMES_AND_NUMBERS]
                    = new lang_string('filterboth', 'format_muonline');
            }
            $courseformatoptionsedit['displayfilterbar'] = [
                'label' => new lang_string('displayfilterbar', 'format_muonline') . ' ' . $outcomeslink,
                'element_type' => 'select',
            ...
            $courseformatoptionsedit['courseshowtileprogress'] = [
                'label' => new lang_string('courseshowtileprogress', 'format_muonline'),
                'element_type' => 'select',
                'element_attributes' => [
                    [
                        0 => new lang_string('hide', 'format_muonline'),
                        1 => new lang_string('asfraction', 'format_muonline'),
                        2 => new lang_string('aspercentagedial', 'format_muonline'),
                    ],
                ],
    ...
    public function section_format_options($foreditform = false) {
        ...
        $sectionformatoptions['muonlineectionsubtitle'] = [
            'default' => '',
            'type' => PARAM_TEXT,
            'element_type' => 'textarea',
            'element_attributes' => [['rows' => 3, 'cols' => 60]],
            'label' => new lang_string('tilesectionsubtitle', 'format_muonline'),
            'help' => 'tilesectionsubtitle',
            'help_component' => 'format_muonline',
        ];
        $sectionformatoptions['muonlineectionstartdate'] = [
            'default' => 0,
            'type' => PARAM_INT,
            'element_type' => 'date_selector',
            'element_attributes' => [['optional' => true]],
            'label' => new lang_string('tilesectionstartdate', 'format_muonline'),
            'help' => 'tilesectionstartdate',
            'help_component' => 'format_muonline',
        ];
        $sectionformatoptions['muonlineectionenddate'] = [
            'default' => 0,
            'type' => PARAM_INT,
            'element_type' => 'date_selector',
            'element_attributes' => [['optional' => true]],
            'label' => new lang_string('tilesectionenddate', 'format_muonline'),
            'help' => 'tilesectionenddate',
            'help_component' => 'format_muonline',
        ];
        $usingoutcomesfilter = in_array(
            $course->displayfilterbar,
            [
                \format_muonline\local\format_option::FILTER_OUTCOMES_ONLY,
                \format_muonline\local\format_option::FILTER_OUTCOMES_AND_NUMBERS,
            ]
        );
        ...
        if ($foreditform) {
            ...
            $sectionformatoptionsedit = [
                'formatmuonlineignoredfield' => ['label' => '', 'element_type' => 'hidden', 'type' => PARAM_INT, 'default' => 0],
            ];
            ...
            if ($usingoutcomesfilter) {
                ...
                $label = get_string('tileoutcome', 'format_muonline') . ' ' . $outcomeslink;
                $outcomes = \format_muonline\local\filters::get_course_outcomes($course->id);
                if (!empty($outcomes)) {
                    $outcomes[0] = get_string('none', 'format_muonline');
                }
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        if (!$forsection) {
            $jsparams = [
                ...
                'courseId' => $COURSE->id,
                get_config('format_muonline', 'allowphotomuonline'),
                'activitydocsurl' => get_docs_url('Activity_completion_settings'),
            ];
            $PAGE->requires->js_call_amd('format_muonline/edit_form_helper', 'init', $jsparams);
        } else {
            // Add a tip to the edit section form for anyone who does not know how to edit tile icon/photos.
            $mform->addElement(
                'html',
                html_writer::div(
                    html_writer::div(get_string('setbackgroundphoto', 'format_muonline'), 'col-md-3 col-form-label')
                    . html_writer::div(get_string('tileselecttip', 'format_muonline'), 'col-md-9'),
                    'row mb-3'
                )
            );
        }
     ...
     *
     * If course format was changed to 'muonline', we try to copy options
     * from the previous format.  We do not copy 'coursedisplay',
     * and 'hiddensections' as a defaut value of one makes sense for these for muonline format,
    ...
    public function update_course_format_options($data, $oldcourse = null) {
                ...
                if ($oldcourse !== null && $oldcourse['format'] == 'grid') {
                    ...
                    if ($dbman->table_exists($gridtable)) {
                        ...
                        foreach ($gridformaticons as $gridformaticon) {
                            ...
                            $tilephoto = new \format_muonline\local\tile_photo($coursecontext, $gridformaticon->sectionid);
                            ...
                            if ($gridfile && !$existingtilephoto) {
                                // We copy the grid image file into Muonline format, so it is included in backups etc.
                                $fs = get_file_storage();
                                $newfilerecord = \format_muonline\local\tile_photo::file_api_params();
        ...
        // Subtiles are always enabled for this format; keep the values forced on regardless of legacy data.
    ...
    public function update_section_format_options($data) {
        ...
        $oldvalues = [
            'outcomethistile' => $DB->get_record(
                ...
                ['courseid' => $this->courseid, 'format' => 'muonline', 'sectionid' => $data['id'], 'name' => 'tileoutcomeid']
            ),
        ];

        // If the edit is taking place from format_muonline_inplace_editable(),
        ...
        foreach (['muonlineectionstartdate', 'muonlineectionenddate'] as $dateoption) {
            if (isset($data[$dateoption]) && (int)$data[$dateoption] <= 0) {
                unset($data[$dateoption]);
            }
        }
        ...
        $keystoremove = ['tileoutcomeid', 'muonlineectionstartdate', 'muonlineectionenddate'];
        foreach ($keystoremove as $key) {
            if (!isset($data[$key])) {
                $DB->delete_records(
                    'course_format_options',
                    ['courseid' => $this->courseid, 'format' => 'muonline', 'sectionid' => $data['id'], 'name' => $key]
                );
    ...
    public function inplace_editable_render_section_name(
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_muonline');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_muonline', $title);
        }
    ...
    public function section_action($section, $action, $sr) {
        ...
        if ($section->section && ($action === 'setmarker' || $action === 'removemarker')) {
            // Format 'muonline' allows to set and remove markers in addition to common section actions.
    ...
    public function page_set_course(moodle_page $page) {
        global $SESSION, $DB;
        $muonlineactionparam = optional_param('format-muonline-action', '', PARAM_TEXT);
        if ($muonlineactionparam) {
            require_sesskey();
        }
        if ($muonlineactionparam == 'toggleanimatednav') {
            if (get_config('format_muonline', 'usejavascriptnav')) {
                // User is toggling JS nav setting.
                $userpreferencenamejsnav = 'format_muonline_stopjsnav';
                ...
                if (!$existingstoppref) {
                    ...
                    \core\notification::warning(get_string('jsdeactivated', 'format_muonline'));
                } else {
                    // User previously disabled it, but now is re-enabling.
                    unset_user_preference($userpreferencenamejsnav);
                    \core\notification::success(get_string('jsreactivated', 'format_muonline'));
                }
                if ($page->course->id ?? null) {
                    redirect(new moodle_url('/course/view.php', ['id' => $page->course->id]));
                }
                unset($SESSION->format_muonline_jssuccessfullyused);
            }
        } else if ($muonlineactionparam == 'togglehighcontrast') {
            if (get_config('format_muonline', 'highcontrastmodeallow')) {
                // User is toggling high contrast setting.
                $userpreferencenamecontrast = 'format_muonline_high_contrast_mode';
        ...
        if ($page->state <= $page::STATE_BEFORE_HEADER) {
            // On a single section page in non JS mode, if not using sub-muonline, do not remove core limited page width.
            if ($page->pagetype == 'course-section' && !\format_muonline\local\util::using_js_nav()) {
                $courseusessubmuonline = get_config('format_muonline', 'allowsubmuonlineview')
                    && ($page->course->id ?? null)
                    && $DB->get_field(
                        'course_format_options',
                        'value',
                        ['courseid' => $page->course->id, 'format' => 'muonline', 'sectionid' => 0, 'name' => 'courseusesubmuonline']
                    ) == "1";
                if (!$courseusessubmuonline) {
                    $page->add_body_class("format-muonline-single-sec");
                }
            }
            if (\format_muonline\local\util::using_high_contrast()) {
                $page->add_body_class("format-muonline-high-contrast");
            }
        }
    }
 ...
 * @throws dml_exception
 */
function format_muonline_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
 ...
 */
function format_muonline_get_fontawesome_icon_map() {
    $iconset = new format_muonline\local\icon_set();
 ...
 * @return void
 */
function format_muonline_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    ...
    $fileapiparams = \format_muonline\local\tile_photo::file_api_params();
    $fs = get_file_storage();
 ...
 */
function format_muonline_output_fragment_get_cm_list(array $args): string {
    global $PAGE, $DB;
    ...
    if ($course->format != 'muonline') {
        throw new moodle_exception(
            'invalidsectionid',
            'format_muonline',
            '',
            $sectionid,
            "Course '$course->id' is not a muonline course"
        );
    }
    ...
    try {
        ...
        // Add this to avoid equations failing when muonline are closed/re-opened.
    ...
    $renderer = $PAGE->get_renderer('format_muonline');
    $templateable = new \format_muonline\output\course_output($course, true, $section->section);
    $data = $templateable->export_for_template($renderer);

    $template = $section->section == 0 ? 'format_muonline/section_zero' : 'format_muonline/single_section_content';
 ...
 * @return string The HTML to add to page.
 */
function format_muonline_output_fragment_get_cm_content(array $args): string {
    global $DB, $CFG, $PAGE;
    ...
    if ($mod) {
        $allowedmodules = explode(",", get_config('format_muonline', 'modalmodules'));
        ...
        if ($mod->modname == 'page') {
            ...
            return \format_muonline\local\util::format_cm_content_text($mod->modname, $record, $modcontext);
        }
        if ($treataslabel) {
