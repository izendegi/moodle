...
class image_processor {
    ...
    public static function adjust_and_copy_file(
        ...
        try {
            ...
            $tmproot = make_temp_directory('formatmuonlinephoto');
            ...
            if (!empty($data)) {
                ...
                debugging('imagecannotbeused', 'format_muonline', DEBUG_DEVELOPER);
            }
        } catch (\Exception $e) {
            if (isset($tempfile)) {
                unset($tempfile);
            }
            debugging('Format muonline image exception: ' . $e->getMessage(), DEBUG_DEVELOPER);
            return false;
        }
        return false;
    }
