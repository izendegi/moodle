...
/**
 * General utilities class for format_muonline.
...
class util {
    ...
    public static function get_course_mod_info(int $courseid, int $cmid): ?object {
        ...
        $modaltype = \format_muonline\local\modal_helper::cm_modal_type($courseid, $cmrecord->id);
    ...
    /**
     * Get the release details of this version of Muonline.
     * @return string
     */
    public static function get_muonline_plugin_release(): string {
        global $CFG;
        $plugin = new \stdClass();
        $plugin->release = '';
        require("$CFG->dirroot/course/format/muonline/version.php");
    ...
    /**
     * Is the user using JS navigation i.e. animated muonline?
     * @return bool
     * @throws \coding_exception
     * @throws \dml_exception
     */
    public static function using_js_nav() {
        ...
        return get_config('format_muonline', 'usejavascriptnav')
            && !get_user_preferences('format_muonline_stopjsnav', 0)
    ...
    public static function using_tile_fitter(): bool {
        ...
        if (optional_param('skipcheck', 0, PARAM_INT)) {
            ...
            $SESSION->format_muonline_skip_width_check = 1;
            return false;
        }

        return get_config('format_muonline', 'fitmuonlinetowidth')
            && \core_useragent::get_device_type() != \core_useragent::DEVICETYPE_MOBILE
            && ($SESSION->format_muonline_skip_width_check ?? null) != 1;
    }
    ...
    public static function get_tile_fitter_max_width(int $courseid): int {
        ...
        $var = 'format_muonline_width_' . $courseid;
     ...
     * @throws \dml_exception
     */
    public static function get_muonline_palette() {
        $palette = [];
        for ($i = 1; $i <= 10; $i++) {
            $colourname = get_config('format_muonline', 'colourname' . $i);
            $tilecolour = get_config('format_muonline', 'tilecolour' . $i);
    ...
    /**
     * Include AMD module required for muonline course.
    ...
    public static function init_js($course, int $contextid, ?int $sectionnumber) {
        ...
        if (!$PAGE->user_is_editing()) {
            ...
            $jsparams = [
                ...
                'assumeDataStoreContent' => get_config('format_muonline', 'assumedatastoreconsent'),
                'reOpenLastSection' => get_config('format_muonline', 'reopenlastsection'),
                'userId' => $USER->id,
                'fitMuonlineToWidth' => self::using_tile_fitter(),
                'enablecompletion' => $course->enablecompletion,
                'usesubmuonline' => get_config('format_muonline', 'allowsubmuonlineview') && ($course->courseusesubmuonline ?? false),
                'courseContextId' => $contextid,
            ];
            $PAGE->requires->js_call_amd('format_muonline/course', 'init', $jsparams);
        } else {
            // Initialise JS for when editing mode is on.
            $editparams = [
                'courseId' => $course->id,
                'pageType' => $PAGE->pagetype,
                'allowPhotoMuonline' => get_config('format_muonline', 'allowphotomuonline'),
                'maxnumbericons' => self::get_icon_picker_max_number_icons(),
            ];
            $PAGE->requires->js_call_amd('format_muonline/edit_icon_picker', 'init', $editparams);
        }
        if ($course->enablecompletion) {
            $PAGE->requires->js_call_amd('format_muonline/completion', 'init', [$course->id]);
        }
    }
    ...
    public static function get_js_config_data(int $courseid) {
        ...
        if (get_config('format_muonline', 'usecourseindex')) {
            $allowedmodals = \format_muonline\local\modal_helper::allowed_modal_modules();
            ...
            $jsconfigvalues['modalAllowedCmids'] = json_encode(
                \format_muonline\local\modal_helper::get_modal_allowed_cm_ids_integer_list($courseid, true)
            );
        }
        ...
        $jsconfigvalues['defaultcourseicon'] = $DB->get_field(
            ...
            'value',
            ['courseid' => $courseid, 'format' => 'muonline', 'sectionid' => 0, 'name' => 'defaulttileicon']
        );
    ...
    public static function using_high_contrast(): bool {
        return get_config('format_muonline', 'highcontrastmodeallow')
            && get_user_preferences('format_muonline_high_contrast_mode', 0);
    }
