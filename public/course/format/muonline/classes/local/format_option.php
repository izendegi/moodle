...
/**
 * Format option class for format muonline.
...
class format_option {
    /**
     * When we store the photo in db format_muonline_tile_options table, specify if it's a section or cm photo.
     */
    ...
    /**
     * Muonline filter bar course setting - show no filters.
     */
    const FILTER_NONE = 0;

    /**
     * Muonline filter bar course setting - show tile number filter buttons only.
     */
    const FILTER_NUMBERS_ONLY = 1;

    /**
     * Muonline filter bar course setting - show outcome filter buttons only.
     */
    const FILTER_OUTCOMES_ONLY = 2;

    /**
     * Muonline filter bar course setting - show filter buttons for tile numbers and outcomes.
     */
    const FILTER_OUTCOMES_AND_NUMBERS = 3;

    /**
     * Get a course format option full record from the format_muonline_tile_options table.
     * @param int $courseid
    ...
    public static function get_db_record(int $courseid, int $optiontype, int $elementid) {
        ...
        try {
            ...
            return $DB->get_record(
                'format_muonline_tile_options',
                ['courseid' => $courseid, 'optiontype' => $optiontype, 'elementid' => $elementid]
            );
        } catch (\Exception $e) {
            debugging('Could not get record from db format_muonline_tile_options table ' . $e->getMessage(), DEBUG_DEVELOPER);
            return false;
        }
    }
    ...
    public static function fill_caches(int $courseid, int $optiontype): array {
        ...
        if (!$DB->get_manager()->table_exists('format_muonline_tile_options')) {
            return [];
        }

        $rs = $DB->get_recordset_sql(
            "SELECT elementid, optionvalue
                FROM {format_muonline_tile_options} WHERE courseid = ? AND optiontype = ?
                ORDER BY elementid",
            [$courseid, $optiontype]
        );
        ...
        $elementids = [];
        if ($rs->valid()) {
            $cacheoptions = \cache::make('format_muonline', 'formatoptions');
            ...
            $cacheids = \cache::make('format_muonline', 'formatoptionelementids');
    ...
    /**
     * Get a course format option value from the format_muonline_tile_options table.
     * @param int $courseid
     * @param int $optiontype
    ...
    public static function get(int $courseid, int $optiontype, int $elementid): ?string {
        ...
        $cache = \cache::make('format_muonline', 'formatoptions');
    ...
    /**
     * Unset a course format option value in the format_muonline_tile_options table.
     * @param int $courseid
     * @param int $optiontype
     * @param int $elementid
     * @return bool
    ...
    public static function unset(int $courseid, int $optiontype, int $elementid) {
        ...
        try {
            ...
            if ($record) {
                self::clear_caches($courseid, $optiontype);
                return $DB->delete_records('format_muonline_tile_options', ['id' => $record->id]);
            }
        } catch (\Exception $e) {
    ...
    public static function unset_multiple_types(int $courseid, int $elementid, array $optiontypes): bool {
        ...
        foreach ($optiontypes as $optiontype) {
            ...
            if ($DB->delete_records('format_muonline_tile_options', $params)) {
    ...
    /**
     * Unset all course format option values in the format_muonline_tile_options table for a course.
     * @param int $courseid
     * @return bool
     */
    public static function unset_all_course(int $courseid): bool {
        global $DB;
        try {
            $result = $DB->delete_records('format_muonline_tile_options', ['courseid' => $courseid]);
            self::clear_caches_course($courseid);

            // Delete legacy tile icon choices.
            return $result && $DB->delete_records_select(
                'course_format_options',
                "courseid = :courseid AND format = 'muonline' AND name IN('tilephoto', 'tileicon')",
                ['courseid' => $courseid]
            );
        } catch (\Exception $e) {
            debugging('Could not unset records ' . $e->getMessage(), DEBUG_DEVELOPER);
    ...
    /**
     * Set a course format option in the format_muonline_tile_options table.
     * @param int $courseid
     * @param int $optiontype
     * @param int $elementid
    ...
    public static function set(int $courseid, int $optiontype, int $elementid, string $optionvalue): bool {
        ...
        if ($optionvalue) {
            ...
            if ($DB->insert_record('format_muonline_tile_options', $record)) {
                self::clear_caches($courseid, $optiontype);
                return true;
            }
        }
    ...
    /**
     * Validate an option type for format_muonline_tile_options table.
     * @param int $optiontype
     * @return bool
     * @throws \moodle_exception
     */
    public static function validate_option_type(int $optiontype) {
        ...
        throw new \moodle_exception('invalidargument', 'format_muonline', '', '', 'Unexpected option type ' . $optiontype);
    }
    ...
    public static function migrate_legacy_format_option_type(int $courseid, int $optiontype, int $sectionid = 0) {
        ...
        // If we have any legacy course format options which need moving to format_muonline_tile_options table, move them.
    ...
    public static function get_legacy_format_options_recordset(
        ...
        $queryparams = ['format' => 'muonline', 'courseid' => $courseid];
    ...
    /**
     * Following Muonline plugin 4.3 upgrade, cron should migrate tile icons and photos to a new database table.
     * @see \format_muonline\task\migrate_legacy_data
     * @param int $courseid
     * @return bool
     * @throws \dml_exception
     */
    public static function needs_migration_incomplete_warning(int $courseid): bool {
        global $DB, $SESSION;
        $key = "muonline-migr-warn-$courseid";
        ...
        $hasmigratedoptions = $DB->record_exists_sql(
            "SELECT id FROM {format_muonline_tile_options} WHERE courseid = ? AND optiontype IN (?, ?)",
        ...
        $haslegacyoptions = $DB->record_exists_sql(
            "SELECT id FROM {course_format_options}
                WHERE courseid = ? and format = 'muonline' AND name IN ('tilephoto', 'tileicon')",
            [$courseid]
        );
        if (!$haslegacyoptions) {
            $SESSION->$key = false;
        ...
        if ($legacyoptions->valid()) {
            foreach ($legacyoptions as $legacyoption) {
                ...
                $newrecordexists = $DB->record_exists_sql(
                    "SELECT id FROM {format_muonline_tile_options}
