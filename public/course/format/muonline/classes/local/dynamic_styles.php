...
/**
 * Prepares CSS for Muonline dynamic styles (e.g. course specific colours).
 *
...
class dynamic_styles {
    ...
    public static function page_needs_dynamic_css(): bool {
        ...
        $allowedpagetypes = ['course-view-muonline', 'section-view-muonline', 'course-view-section-muonline'];
    ...
    /**
     * Get the muonline dynamic course CSS to be added to <head>.
     * @return string
     * @throws \coding_exception
     * @throws \dml_exception
     * @throws \moodle_exception
     */
    public static function get_muonline_dynamic_css(): string {
        global $CFG, $PAGE;
        $courseid = $PAGE->course->id ?? 0;
        $data = self::data_for_template($courseid);
        $m = new \Mustache_Engine();
        return $m->render(
            file_get_contents("$CFG->dirroot/course/format/muonline/templates/dynamic_styles.mustache"),
            $data
        );
    }
     ...
     * @see \format_muonline\local\util::width_template_data()
     * @param int $courseid
     * @return array
     * @throws \dml_exception
     * @throws \moodle_exception
    ...
    public static function data_for_template(int $courseid): array {
        ...
        $muonlinetyle = get_config('format_muonline', 'muonlinetyle') ?? \format_muonline\output\course_output::TILE_STYLE_STANDARD;
        $usinghighcontrast = \format_muonline\local\util::using_high_contrast();

        $outputdata = [
            'courseid' => $courseid,
            "isstyle-$muonlinetyle" => true,
            'isstyle1or2' => $muonlinetyle == 1 || $muonlinetyle == 2,
            'base_colour_rgb' => $usinghighcontrast ? "0,0,0" : ($basecolourhex ? self::rgbcolour($basecolourhex) : null),
            'high_contrast_black' => '#000',
            'usesubmuonline' => $course->courseusesubmuonline ?? false,
...
'shade_heading_bar' => 1,
            ...
            'pluginconfigcss' => trim(get_config('format_muonline', 'customcss') ?? ''),
        ];

        if (get_config('format_muonline', 'allowphotomuonline')) {
            $outputdata['allowphotomuonline'] = 1;
            $outputdata['photo_tile_text_bg_opacity'] = $usinghighcontrast ? 1 :
                1.0 - (float)get_config('format_muonline', 'phototiletitletransarency');
            ...
            $outputdata['phototiletextpadding'] = number_format(
                (float)get_config('format_muonline', 'phototitletitlepadding') / 10,
                1
            );
            $outputdata['phototiletextlineheight'] = number_format(
                (float)get_config('format_muonline', 'phototitletitlelineheight') / 10,
                1
            );
        }

        // Tile fitter if used.
        $outputdata['usingtilefitter'] = \format_muonline\local\util::using_tile_fitter();
        $outputdata['tilefittermaxwidth'] = $outputdata['usingtilefitter']
            ? \format_muonline\local\util::get_tile_fitter_max_width($courseid) : 0;
    ...
    public static function get_tile_base_colour($coursebasecolour): string {
        ...
        if (!(get_config('format_muonline', 'followthemecolour'))) {
            if (!$coursebasecolour) {
                ...
                $result = get_config('format_muonline', 'tilecolour1');
            } else {
        ...
        if (!$result || !preg_match($hexpattern, $result)) {
            ...
            $result = get_config('format_muonline', 'tilecolour1') ?? self::DEFAULT_COLOUR;
        }
        return $result;
    }
    ...
    public static function page_needs_loading_icon(int $courseid): bool {
        if (!\format_muonline\local\util::using_tile_fitter()) {
            return false;
        }
        return !\format_muonline\local\util::get_tile_fitter_max_width($courseid);
    }
}
