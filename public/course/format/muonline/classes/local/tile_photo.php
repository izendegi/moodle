...
/**
 * Tile photo class for format muonline.
...
class tile_photo {
    ...
    public function __construct(\context $context, int $sectionid) {
        ...
        if ($this->context->contextlevel === CONTEXT_COURSE) {
            ...
            $this->muonlineoptiontype = format_option::OPTION_SECTION_PHOTO;
        } else {
            debugging(
    ...
    public function get_filename() {
        if ($this->filename === '') {
            ...
            $this->filename = $this->courseid
                ? format_option::get($this->courseid, $this->muonlineoptiontype, $this->get_element_id()) : null;
        }
    ...
    public function set_filename($filename) {
        ...
        format_option::set($this->courseid, $this->muonlineoptiontype, $this->get_element_id(), $this->filename);
    }
    ...
    public function set_file($file) {
        ...
        if ($this->muonlineoptiontype == format_option::OPTION_SECTION_PHOTO) {
            $DB->get_record(
                'course_sections',
    ...
    /**
     * Get the element id for this object as used in the format_muonline_tile_options table.
    ...
    public function verify_aspect_ratio() {
        ...
        $messageshort = get_string('imagesize', 'format_muonline') . ": ";
        if (abs($ratio - $requiredratio) > $tolerance) {
            if ($ratio > $requiredratio) {
                $tallorwide = ['tallorwide' => get_string('tootall', 'format_muonline')];
                $messageshort .= get_string('tootall', 'format_muonline');
            } else {
                $tallorwide = ['tallorwide' => get_string('toowide', 'format_muonline')];
                $messageshort .= get_string('toowide', 'format_muonline');
            }
            return [
                'status' => false,
                'message' => get_string(
        ...
        $messageshort .= get_string('ok', 'format_muonline');
    ...
    public function clear() {
        ...
        format_option::unset($this->courseid, $this->muonlineoptiontype, $this->get_element_id());
    }

    /**
     * Delete all muonline photos and icon choices (can be called by site admin from course nav).
     * @param int $courseid the id for this course.
     * @return bool whether successful.
     */
    public static function reset_muonline_course($courseid) {
