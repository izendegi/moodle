...
/**
 * Icon set class for format muonline.
...
class icon_set {
    /**
     * These are shown on muonline.  The teacher chooses which one to apply to each tile or course.
    ...
    public function available_tile_icons($courseid = 0) {
        ...
        if ($this->supports_font_awesome()) {
            foreach ($this->fontawesometileicons as $iconname) {
                ...
                if ($stringmanager->string_exists('icontitle-' . $pixname, 'format_muonline')) {
                    $displayname = get_string('icontitle-' . $pixname, 'format_muonline');
                } else {
        ...
        $iconsindirectory = get_directory_list($CFG->dirroot
            . '/course/format/muonline/pix/tileicon', '', false, false, true);
        foreach ($iconsindirectory as $icon) {
            ...
            if (!isset($availableicons[$filename])) {
                if ($stringmanager->string_exists('icontitle-' . $filename, 'format_muonline')) {
                    $displayname = get_string('icontitle-' . $filename, 'format_muonline');
                } else {
     ...
     * @see format_muonline_get_fontawesome_icon_map()
     * @return array
     */
    public function get_font_awesome_icon_map() {
        ...
        // First the general icons (not specific to muonline).
        // These are used for example to show nav buttons within muonline.
        $generalicons = [
            'format_muonline:camera' => 'fa-camera',
            'format_muonline:check' => 'fa-check',
            'format_muonline:chevron-left' => 'fa-chevron-left',
            'format_muonline:chevron-right' => 'fa-chevron-right',
            'format_muonline:chevron-up' => 'fa-chevron-up',
            'format_muonline:clone' => 'fa-clone',
            'format_muonline:close' => 'fa-close',
            'format_muonline:cloud-download' => 'fa-cloud-download',
            'format_muonline:cloud-upload' => 'fa-cloud-upload',
            'format_muonline:completion-check' => 'fa-check',
            'format_muonline:completion-fail' => 'fa-times',
            'format_muonline:eye-slash' => 'fa-eye-slash',
            'format_muonline:filter' => 'fa-filter',
            'format_muonline:home' => 'fa-home',
            'format_muonline:image' => 'fa-image',
            'format_muonline:lock' => 'fa-lock',
            'format_muonline:pencil' => 'fa-pencil',
            'format_muonline:random' => 'fa-random',
            'format_muonline:sort' => 'fa-sort',
            'format_muonline:toggle-off' => 'fa-toggle-off',
            'format_muonline:toggle-on' => 'fa-toggle-on',
        ];

        $tileicons = [];
        foreach ($this->fontawesometileicons as $icon) {
            $pixname = str_replace('fa-', '', $icon);
            $tileicons['format_muonline:tileicon/' . $pixname] = $icon;
        }
