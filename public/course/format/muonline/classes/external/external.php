...
namespace format_muonline\external;
...
/**
 * Format muonline external functions
 *
...
class external extends external_api {
        ...
        switch ($data['imagetype']) {
            case 'tileicon':
                if ($data['sectionid'] === 0) {
                    ...
                    return [
                        'status' => (bool)$result,
                        'iconurl' => $result ? $OUTPUT->image_url('tileicon/' . $data['image'], 'format_muonline')->out() : '',
                    ];
                }
            case 'tilephoto':
                if (!get_config('format_muonline', 'allowphotomuonline')) {
                    throw new invalid_parameter_exception("Photo muonline are disabled by site admin");
                }
    ...
    private static function set_tile_photo($data, $context): array {
        ...
        if (!$data['image']) {
            ...
            \format_muonline\local\tile_photo::delete_files_from_ids($data['courseid'], $sectionid);

            // If there is an icon attached to this element, clear it (as in this function we are setting a photo).
            \format_muonline\local\format_option::unset(
                $data['courseid'],
                \format_muonline\local\format_option::OPTION_SECTION_ICON,
                $sectionid
            );
    ...
    private static function set_tile_icon($data, $context) {
        ...
        $tilenumber = \format_muonline\local\util::get_tile_number_from_icon_name($data['image']);
        if (!$tilenumber) {
            $availableicons = (new \format_muonline\local\icon_set())->available_tile_icons($data['courseid']);
            if (!isset($availableicons[$data['image']])) {
                throw new invalid_parameter_exception('Icon is invalid');
            }
        }

        $optiontype = \format_muonline\local\format_option::OPTION_SECTION_ICON;
        $sectionid = $data['sectionid'];
        ...
        $defaulticonthiscourse = $DB->get_field(
            ...
            'value',
            ['courseid' => $data['courseid'], 'format' => 'muonline', 'sectionid' => 0, 'name' => 'defaulttileicon']
        );
    ...
    public static function set_default_tileicon($data) {
        ...
        $existingicon = $DB->get_record(
            'course_format_options',
            ['courseid' => $data['courseid'], 'format' => 'muonline', 'sectionid' => $data['sectionid'], 'name' => $optionname]
        );
        if (!isset($existingicon->value)) {
            // No icon is presently stored for this so we need to insert new record.
            $record = new \stdClass();
            $record->courseid = $data['courseid'];
            $record->format = 'muonline';
    ...
    public static function log_tile_click(int $coursecontextid, int $sectionnumber, int $sectionid) {
        ...
        $SESSION->format_muonline_jssuccessfullyused = 1;
    ...
    public static function get_icon_set($courseid) {
        ...
        $data = [
            'status' => true,
            'warnings' => [],
            'icons' => json_encode((new \format_muonline\local\icon_set())->available_tile_icons($params['courseid'])),
            'photos' => '',
        ];
        if (get_config('format_muonline', 'allowphotomuonline')) {
    ...
    /**
     * Set the result of the JS calculation of the optimal width of the main muonline window for a course.
     * This has to be by course as they have different numbers of muonline.
     ...
     * @see \format_muonline\local\util::width_template_data() for where this is used.
     * @return array of warnings and status result
     * @since Moodle 3.0
     * @throws moodle_exception
     */
    public static function set_session_width($courseid, $width) {
        ...
        self::validate_context($coursecontext);
        $sessionvar = 'format_muonline_width_' . $params['courseid'];

        if (!get_config('format_muonline', 'fitmuonlinetowidth')) {
            throw new invalid_parameter_exception("Setting muonline width is disabled by site admin");
        }
        ...
        return new external_function_parameters(
            [
                ...
                'width' => new external_value(
                    PARAM_INT,
                    'The JS calculated width optimal width for muonline window (used to render from PHP next time)',
                    VALUE_DEFAULT,
                    0,
                    true
                ),
     ...
     * The data provided enable the muonline to be updated client side with progress info and availability.
     * @param int $courseid
    ...
    public static function get_section_information($courseid, $sectionnums) {
        ...
        $templateable = new \format_muonline\output\course_output($course, true);
    ...
    /**
     * Get information used by muonline format about a course module.
     * @param int $cmid
     * @return object
    ...
    public static function get_course_mod_info(int $cmid) {
        ...
        $data = \format_muonline\local\util::get_course_mod_info($context->get_course_context()->instanceid, $params['cmid']);
        if ($data) {
            return $data;
        }
