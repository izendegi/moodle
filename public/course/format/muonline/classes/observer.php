...
use format_muonline\local\modal_helper;
use format_muonline\local\video_cm;
...
class observer {
    ...
    public static function course_deleted(\core\event\course_deleted $event) {
        ...
        \format_muonline\local\tile_photo::delete_files_from_ids($courseid, -1);
        \format_muonline\local\format_option::unset_all_course($courseid);
    ...
    public static function course_section_deleted(\core\event\course_section_deleted $event) {
        \format_muonline\local\tile_photo::delete_files_from_ids($event->courseid, $event->objectid);
        \format_muonline\local\format_option::unset_multiple_types(
            $event->courseid,
            $event->objectid,
    ...
    public static function course_backup_created(\core\event\course_backup_created $event) {
        ...
        if ($event->other['type'] == 'course') {
            $ismuonlinecourse = $DB->record_exists('course', ['id' => $event->objectid, 'format' => 'muonline']);
            if ($ismuonlinecourse) {
                \format_muonline\local\format_option::delete_legacy_format_options($event->objectid);
            }
        }
    }
