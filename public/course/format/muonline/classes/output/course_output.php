...
/**
 * Muonline course format, main course output class to prepare data for mustache templates
 *
 * @package format_muonline
 * @copyright 2018 David Watson {@link http://evolutioncode.uk}
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
namespace format_muonline\output;

use format_muonline\local\format_option;
use format_muonline\local\tile_photo;
use format_muonline\local\filters;
use format_muonline\local\util;
...
class course_output implements \renderable, \templatable {
    ...
    public function __construct($course, $fromajax = false, $sectionnum = null, \renderer_base|null $courserenderer = null) {
        ...
        $this->modalscmids = \format_muonline\local\modal_helper::get_modal_allowed_cm_ids_integer_list(
            $this->course->id,
            false
        );
        ...
        $this->usingjsnav = \format_muonline\local\util::using_js_nav();
    }
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        $processsectionzero = $onmultisectionpage
            || (get_config('format_muonline', 'showseczerocoursewide') && !$this->isediting)
            || $this->sectionnum === '0';
    ...
    protected function get_basic_data() {
        ...
        $data['showJScancelLink'] = isset($SESSION->format_muonline_jssuccessfullyused) ? 1 : 0;
        $data['editing'] = $this->isediting;
        $data['sesskey'] = sesskey();
        $data['showinitialpageloadingicon'] = !$this->isediting
            && \format_muonline\local\dynamic_styles::page_needs_loading_icon($this->course->id);
        $data['jsnavadminallowed'] = get_config('format_muonline', 'usejavascriptnav');
        $data['usingjsnav'] = $this->usingjsnav ? 1 : 0;
        $data['jsnavuserenabled'] = !get_user_preferences('format_muonline_stopjsnav');
        $data['abovemuonlinecontrols'] = $this->get_above_muonline_controls($data['jsnavadminallowed'], $data['usingjsnav']);
        $data['hasabovemuonlinecontrols'] = !empty($data['abovemuonlinecontrols']);

        $data['useSubmuonline'] = get_config('format_muonline', 'allowsubmuonlineview') && $this->courseformatoptions['courseusesubmuonline'];
        $data['outerextraclasses'] = get_config('format_muonline', 'subtileiconcolourbackground')
            ? 'format-muonline-colour-subtile-icon-bg' : '';
        ...
        if ($data['canedit'] && format_option::needs_migration_incomplete_warning($this->course->id)) {
            $message = get_string('coursephotomigrationincomplete', 'format_muonline');
            $message .= \html_writer::link(
                new \moodle_url('/course/format/muonline/editor/migratecoursedata.php', ['courseid' => $this->course->id]),
                '<i class="icon fa fa-cog me-1"></i>' . get_string('fixproblems', 'format_muonline'),
                ['class' => 'ms-1']
            );
            $data['editorwarnings'][] = [
                'text' => $message,
    ...
    protected function temp_course_section_cm_availability($mod) {
        if ($this->courseformatoptions['courseusesubmuonline']) {
            // Submuonline show a badge on the tile including full info.
    ...
    protected function append_section_zero_data($data, $output) {
        ...
        $data['section_zero']['is_collapsible'] = get_config('format_muonline', 'seczerocollapsible');

        if ($this->courseformatoptions['courseusesubmuonline'] && $this->courseformatoptions['usesubmuonlineseczero']) {
            $data['section_zero']['useSubmuonline'] = 1;
        } else {
            $data['section_zero']['useSubmuonline'] = 0;
        }
        return $data;
    }
    ...
    protected function get_course_format_options($fromajax) {
        ...
        $options = [
            'defaulttileicon', 'basecolour', 'courseshowtileprogress',
            'displayfilterbar',
        ];
        $data = [];
        if (!$fromajax) {
        ...
        $data['courseusesubmuonline'] = 1;
        $data['usesubmuonlineseczero'] = 1;
        return $data;
    }
    ...
    protected function append_single_section_page_data($output, $data) {
        ...
        if (get_config('format_muonline', 'enablelinebreakfilter')) {
        ...
        if (get_config('format_muonline', 'allowphotomuonline')) {
            ...
            if ($tilephotourl) {
                ...
                $data['phototileediturl'] = new \moodle_url(
                    '/course/format/muonline/editor/editimage.php',
        ...
        $data['useSubmuonline'] = get_config('format_muonline', 'allowsubmuonlineview')
            && $this->courseformatoptions['courseusesubmuonline']
            && ($thissection->section === 0
                ? $this->courseformatoptions['usesubmuonlineseczero']
                : false);
    ...
    protected function append_multi_section_page_data($data) {
        ...
        // Before we start the section loop. get key vars for photo muonline ready.
        $allowedphotomuonline = get_config('format_muonline', 'allowphotomuonline');
        $muonlinetyle = get_config('format_muonline', 'muonlinetyle') ?? self::TILE_STYLE_STANDARD;
        $data['isstyle-' . $muonlinetyle] = true;
        $data['muonlinetyle'] = $muonlinetyle;

        if ($allowedphotomuonline) {
            $data['allowphotomuonline'] = 1;
            $data['showprogressphotomuonline'] = get_config('format_muonline', 'showprogresssphotomuonline');
        ...
        $uselinebreakfilter = get_config('format_muonline', 'enablelinebreakfilter');
        ...
        foreach ($secsall as $sectionnum => $section) {
            ...
            $isphototile = $allowedphotomuonline && in_array($section->id, $phototileids);
            ...
            if ($sectionnum != 0 && $showsection && !$isdelegated) {
                ...
                $ariatitle = get_string(
                    ...
                    trim($title)
                        ? $title
                        : get_string('tilearialabel', 'format_muonline', $this->format->get_default_section_name($section))
                );
                if ($allowedphotomuonline && $muonlinetyle == self::TILE_STYLE_BOTTOM_TITLE && $isphototile) {
                ...
                $longtitlelength = 65;
                $sectiondates = [];
                $dateformat = $this->get_section_date_format();
                if (!empty($section->muonlineectionstartdate)) {
                    $sectiondates[] = userdate(
                        $section->muonlineectionstartdate,
                        $dateformat
                    );
                }
                if (!empty($section->muonlineectionenddate)) {
                    $sectiondates[] = userdate(
                        $section->muonlineectionenddate,
                        $dateformat
                    );
                }

                $subtitle = $section->muonlineectionsubtitle;
                if ($subtitle !== '') {
                    $subtitle = format_string($subtitle, true, ['context' => $this->coursecontext]);
                }
                ...
                $newtile = [
                    ...
                    'title' => $title,
                    'subtitle' => $subtitle,
                    'sectiondates' => implode(' – ', $sectiondates),
                    ...
                    'extraclasses' => "muonlinetyle-$muonlinetyle ",
        ...
        if ($this->courseformatoptions['displayfilterbar']) {
            ...
            if ($usingnumbersfilter) {
                $data['filternumberedbuttons'] = filters::get_filter_numbered_buttons_data($data['muonline']);
            }
            if ($usingoutcomesfilter) {
                ...
                $data['filteroutcomebuttons'] = filters::get_filter_outcome_buttons_data($data['muonline'], $outcomes, $firstid);
            }
        }
        ...
        if ($this->completionenabled && $data['overall_progress']['num_out_of'] > 0) {
            if (get_config('format_muonline', 'showoverallprogress')) {
    ...
    /**
     * Get the locale-aware date format to use for section dates.
     *
     * @return string
     */
    protected function get_section_date_format(): string {
        $language = current_language();
        if ($language === 'eu' || strpos($language, 'eu_') === 0) {
            return '%B %d';
        }

        return get_string('strftimedateshort', 'langconfig');
    }
     ...
     * Exclude labels if we are using sub muonline, as these are not checkable
    ...
    public function section_progress(int $sectionnum): array {
        ...
        if (!empty($sectioncmids)) {
            ...
            $includesubsectiondata = get_config('format_muonline', 'progressincludesubsections');
     ...
     * This is useful on muonline with long words in the title (e.g. German language).
     * @param string $text
    ...
    protected function course_module_data($mod, $section, $previouswaslabel, $isfirst, $output): array {
        ...
        if (class_exists('\local_modcustomfields\customfield\mod_handler')) {
            try {
                ...
                $customfields = $handler->export_instance_data_object($mod->id);
                $moduleobject['modinfo'] = $customfields->modinfo ?? '';
            } catch (\Exception $e) {
                // Ignore if custom field handler is not available for this mod or field.
            }
        }
        if (!$this->courseformatoptions['courseusesubmuonline'] && $mod->indent) {
        ...
        if ($treataslabel) {
            ...
            if (!$isfirst && !$previouswaslabel && $this->courseformatoptions['courseusesubmuonline']) {
        ...
        if (!$treataslabel) {
            ...
            if (!\format_muonline\local\util::has_monologo_icon('mod', $mod->modname)) {
                // To use the mod's non-monologo icon we need no filtering.
                $iconclass = 'nofilter';
            }

            // Turnitin logo is too small and blurry on submuonline and wrong colour.
            if ($mod->modname === 'turnitintooltwo' && $this->courseformatoptions['courseusesubmuonline']) {
        ...
        if (\format_muonline\local\modal_helper::mod_uses_cm_modal_cache($mod->modname)) {
            $moduleobject['modalType'] = in_array($mod->id, $this->modalscmids)
                ? \format_muonline\local\modal_helper::cm_modal_type($this->course->id, $mod->id)
                : null;
        }

        // Issue 67 handling for LTI set to open in new window.
        // Where onclick is truthy, suggests core JS will open in new window so don't treat as muonline modal.
        if ($mod->onclick) {
        ...
        if ($moduleobject['showdescription']) {
            ...
            if ($moduleobject['modalType'] ?? null == 'pdf' && $this->courseformatoptions['courseusesubmuonline']) {
        ...
        $moduleobject['tags'] = [];
        $moduleobject['hastags'] = false;
        if (\core_tag_tag::is_enabled('core', 'course_modules')) {
            $coursetags = \core_tag_tag::get_item_tags(
                'core',
                'course_modules',
                $mod->id,
            ...
            foreach ($coursetags as $tag) {
                $moduleobject['tags'][] = [
                    ...
                    'viewurl' => (string) $tag->get_view_url(),
                ];
            }
            $moduleobject['hastags'] = !empty($moduleobject['tags']);
        }
        ...
        if ($mod->modname == 'url' && \format_muonline\local\video_cm::is_video_cm($this->course->id, $mod->id)) {
            $externalurl = $DB->get_field('url', 'externalurl', ['id' => $mod->instance]);
            $modifiedvideourl = \format_muonline\local\video_cm::check_modify_embedded_url($externalurl);
            if ($modifiedvideourl) {
                // Even though it's really a URL activity, display it as "video" activity with video icon.
                $videostring = get_string('displaytitle_mod_mp4', 'format_muonline');
                if ($this->courseformatoptions['courseusesubmuonline']) {
                    $moduleobject['extraclasses'] .= ' video';
                    $moduleobject['modnameDisplay'] = $videostring;
                }
                $moduleobject['icon'] = [
                    'url' => $output->image_url("play", 'format_muonline'),
                    'label' => $videostring,
                ];
            }
        }
            ...
            if ($mod->completion == COMPLETION_TRACKING_MANUAL) {
                ...
                switch ($completiondata->completionstate) {
                    case COMPLETION_INCOMPLETE:
                        $moduleobject['completionstring'] = get_string('togglecompletionincomplete', 'format_muonline');
                        break;
                    case COMPLETION_COMPLETE:
                        $moduleobject['completionstring'] = get_string('togglecompletioncomplete', 'format_muonline');
                        break;
                }
            } else { // Automatic.
                switch ($completiondata->completionstate) {
                    case COMPLETION_INCOMPLETE:
                        $moduleobject['completionstring'] = get_string('complete-n-auto', 'format_muonline');
                        break;
                    case COMPLETION_COMPLETE:
                        $moduleobject['completionstring'] = get_string('complete-y-auto', 'format_muonline');
                        break;
    ...
    /**
     * Get an array of the controls to show above the muonline e.g. high contrast mode.
     ...
     */
    public function get_above_muonline_controls(bool $jsnavadminallowed, bool $usingjsnav): array {
        $controls = [];
        $courseurl = '/course/view.php';
        $courseurlparams = ['id' => $this->course->id, 'sesskey' => sesskey()];
        if ($jsnavadminallowed) {
            $controls[] = [
                'url' => new \moodle_url($courseurl, array_merge($courseurlparams, ['format-muonline-action' => 'toggleanimatednav'])),
                'label' => get_string('jsactivate', 'format_muonline'),
                'checked' => $usingjsnav,
            ];
        }
        if (get_config('format_muonline', 'highcontrastmodeallow')) {
            $controls[] = [
                'url' => new \moodle_url(
                    $courseurl,
                    array_merge($courseurlparams, ['format-muonline-action' => 'togglehighcontrast'])
                ),
                'label' => get_string('highcontrastmode', 'format_muonline'),
                'checked' => \format_muonline\local\util::using_high_contrast(),
            ];
        }
        return $controls;
    }
