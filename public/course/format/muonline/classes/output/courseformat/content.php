...
namespace format_muonline\output\courseformat;


use core_courseformat\output\local\content as content_base;

/**
 * Format muonline class to render course content.
 *
 * @package   format_muonline
 * @copyright 2022 David Watson
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class content extends content_base {
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        if ($isediting) {
            ...
            if (
                get_config('format_muonline', 'allowsubmuonlineview')
                && isset($courseformatoptions['courseusesubmuonline']) && $courseformatoptions['courseusesubmuonline']
            ) {
                // For now (Beta version) we warn editor about sub muonline only appearing in non-edit view.
                $messgage = get_string('editoradvicesubtiles', 'format_muonline');
                if (has_capability('moodle/site:config', \context_system::instance())) {
                    $messgage .= ' ('
                        . get_string('version', 'format_muonline', \format_muonline\local\util::get_muonline_plugin_release()) . ')';
                }
                $data->editoradvice[] = ['text' => $messgage, 'icon' => 'info-circle', 'class' => 'secondary'];

                // Inline folders will not display in sub-muonline view so warn if present.
            ...
            if ($warneditorcompletion) {
                $bulklink = \html_writer::link(
                    ...
                    get_string('completionwarning_changeinbulk', 'format_muonline')
                );
                ...
                $data->editoradvice[] = [
                    'text' => get_string('completionwarning', 'format_muonline') . ' '  . $bulklink . ' ' . $helplink,
                    'icon' => 'info-circle', 'class' => 'secondary',
                ];
            }

            $hasbulkedittools = \format_muonline\local\util::get_moodle_release() >= 4.2
            ...
            if ($moodlerelease >= 4.0 && \format_muonline\local\format_option::needs_migration_incomplete_warning($course->id)) {
                $message = get_string('coursephotomigrationincomplete', 'format_muonline');
                if ($isadmin) {
                    $message .= \html_writer::link(
                        new \moodle_url('/course/format/muonline/editor/migratecoursedata.php'),
    ...
    /**
     * Folders set to display inline will not work this format (if using submuonline).
    ...
    private function get_inline_folder_warning(int $courseid): ?string {
        ...
        if ($inlinefolder) {
            ...
            return get_string('folderdisplayerror', 'format_muonline', $link);
        }
        return null;
    }
}
