...
/**
 * Basic renderer for muonline format.
...
class renderer extends section_renderer {
     ...
     * Used at present to ensure muonline specific editor controls are shown.
     * @return void
     */
    public function render_content() {
        ...
        if ($this->page->user_is_editing()) {
            ...
            $template = 'format_muonline/local/content';
            ...
            if (self::display_multiple_section_page((bool)$sectionnumber, false)) {
                $template = 'format_muonline/multi_section_page';
                $templateable = new \format_muonline\output\course_output($course, false, null, $this);
                $data = $templateable->export_for_template($this);
            } else {
                $template = 'format_muonline/single_section_page';
                $templateable = new \format_muonline\output\course_output($course, false, $sectionnumber, $this);
                $data = $templateable->export_for_template($this);
            }
        }
        ...
        \format_muonline\local\util::init_js($course, $this->page->context->id, $sectionnumber);
     ...
     *
     * In muonline, we override so that when teacher hides/shows tile with AJAX, re-rendered sec includes photo/icon.
     * @see \format_muonline\output\section_renderer::render_section()
     * @see \format_muonline\output\courseformat\content\section\controlmenu::export_for_template()
    ...
    public function course_section_updated(\core_courseformat\base $format, \section_info $section): string {
        ...
        $renderer = new \format_muonline\output\section_renderer($this->page, $this->target);

        // This ends up calling \format_muonline\output\section_renderer::render_section().
        return $renderer->render($output);
    }

    /**
     * Should we display a multiple section page or not?
     * I.e. do we display all muonline on screen or just one open section?
    ...
    private function display_multiple_section_page(bool $displaysection, bool $isediting): bool {
        ...
        // We know they have JS if $SESSION->format_muonline_jssuccessfullyused is set.
        ...
        if (\format_muonline\local\util::using_js_nav() && isset($SESSION->format_muonline_jssuccessfullyused)) {
            if (!$isediting && get_config('format_muonline', 'usejsnavforsinglesection')) {
                return true;
            }
        }
        return false;
     ...
     * We override this here and use existing local code for submuonline pending full refactoring.
    ...
    public function render_delegatedsection($widget) {
        ...
        $templateable = new \format_muonline\output\course_output(
            $this->page->course,
            true,
            $sectionnum
        );
        $data = $templateable->export_for_template($this);
        $template = 'format_muonline/course_modules_subsection';

        // If submuonline are not being used we can use core widget data and template.
        $usecore = !$data['useSubmuonline'] || $this->page->user_is_editing();
        if ($usecore) {
            $data = $parentdata;
            $data->contentcollapsed = true;
            $data->isdelegatedsection = true;
            $template = 'format_muonline/local/content/delegatedsection';
        }
