...
class standard_head_html_prepend {
    /**
     * Callback to add head elements.  Used to add dynamic CSS used by Muonline format.
     *
    ...
    public static function callback(\core\hook\output\before_standard_head_html_generation $hook): void {
        ...
        $debug = "Could not prepare format_muonline head data";
        try {
            if (method_exists('format_muonline\local\dynamic_styles', 'page_needs_dynamic_css')) {
                if (\format_muonline\local\dynamic_styles::page_needs_dynamic_css()) {
                    $dynamiccss = \format_muonline\local\dynamic_styles::get_muonline_dynamic_css();
                    if ($dynamiccss) {
                        $hook->add_html("<style id=\"format-muonline-dynamic-css\">$dynamiccss</style>");
                    }
                }
            } else {
        ...
        try {
            if (\core_useragent::is_safari() || \core_useragent::is_safari_ios()) {
                ...
                $safaricss = file_get_contents("$CFG->dirroot/course/format/muonline/styles_safari.css");
                $hook->add_html("<style id=\"format-muonline-safari-styles\">$safaricss</style>");
            }
        } catch (\Exception $e) {
