@format @format_muonline @format_muonline_course_mod_visibility_nosubmuonline_hidden_sec @javascript
Feature:  In a section for Teacher, hidden activities are dimmed where submuonline are not being used.
  ...
  Background:
    ...
    And the following "courses" exist:
      ...
      | Course Mod Vis NoSubmuonline Course2 | C1        | muonline  | 0             | 5           | 1                |
    ...
    And format_muonline submuonline are off for course "Course Mod Vis NoSubmuonline Course2"

    And  I log in as "teacher1"
    And I am on "Course Mod Vis NoSubmuonline Course2" course homepage with editing mode on
...
#  Section 1 is hidden so all activities below should be dimmed even if not individually hidden
  ...
  @javascript
  Scenario: Teacher sees hidden section but activities are dimmed with submuonline off
    When I log in as "teacher1"

    And I am on "Course Mod Vis NoSubmuonline Course2" course homepage

    And I click on tile "1"
    And I wait until the page is ready
    Then I should see "Test quiz V"
    And activity in format muonline is dimmed "Test quiz V"

    And I should see "Test page V"
    And activity in format muonline is dimmed "Test page V"

    And I should see "Test forum V"
    And activity in format muonline is dimmed "Test forum V"

    And I should see "Test URL V"
    And activity in format muonline is dimmed "Test URL V"

    And I should see "Test quiz NV"
    And activity in format muonline is dimmed "Test quiz NV"

    And I should see "Test page NV"
    And activity in format muonline is dimmed "Test page NV"

    And I should see "Test forum NV"
    And activity in format muonline is dimmed "Test forum NV"

    And I should see "Test URL NV"
    And activity in format muonline is dimmed "Test URL NV"

    And I click on close button for tile "1"
    And I log out muonline

  @javascript
  Scenario: Student cannot see visible (NV) activities or section with submuonline off
