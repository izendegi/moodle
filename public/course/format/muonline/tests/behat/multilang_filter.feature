@format @format_muonline @format_muonline_multi_lang_filter @javascript
Feature: Multi lang filter works on course headings and content
  ...
  @javascript
  Scenario: Activity headings are filtered for submuonline
  ...
  @javascript
  Scenario: Activity headings are filtered for non-submuonline
    When I log in as "student1"
  ...
  Scenario: Page modal content is filtered
    ...
    And I click on tile "1"
    And I wait until the page is ready
    And I click format muonline activity "English page"
    And I wait until the page is ready
