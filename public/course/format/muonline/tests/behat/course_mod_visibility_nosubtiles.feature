@format @format_muonline @format_muonline_course_mod_visibility_nosubmuonline @javascript
Feature: In a section for Teacher, hidden activities are dimmed
  ...
  Background:
    ...
    And the following "courses" exist:
      ...
      | Course Mod Vis NoSubmuonline Course | C1        | muonline  | 0             | 5           | 1                |
    ...
    And format_muonline submuonline are off for course "Course Mod Vis NoSubmuonline Course"

  @javascript
  Scenario: Teacher can see visible (V) and not visible (NV) activities with submuonline off
    When I log in as "teacher1"
    And I am on "Course Mod Vis NoSubmuonline Course" course homepage
    And I click on tile "1"
    And I wait until the page is ready

    Then I should see "Test quiz V"
    And activity in format muonline is not dimmed "Test quiz V"

    And I should see "Test page V"
    And activity in format muonline is not dimmed "Test page V"

    And I should see "Test forum V"
    And activity in format muonline is not dimmed "Test forum V"

    And I should see "Test URL V"
    And activity in format muonline is not dimmed "Test URL V"
  ...
  @javascript
  Scenario: Student can see visible (V) but cannot see invisible (NV) activities with submuonline off
