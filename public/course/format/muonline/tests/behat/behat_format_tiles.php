...
/**
 * Steps definitions related to Format muonline
 *
...
/**
 * Format muonline related steps definitions.
 *
 * @package    format_muonline
 * @category   test
 * @copyright  2018 David Watson {@link http://evolutioncode.uk}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_format_muonline extends behat_base {
    /**
     * Set course format option for submuonline on for course.
     *
     * @Given /^format_muonline submuonline are on for course "(?P<coursefullname_string>(?:[^"]|\\")*)"$/
     * @param string $coursefullname
     * @throws dml_exception
     */
    public function format_muonline_sub_muonline_are_on_for_course($coursefullname) {
        $this->sub_muonline_on_off($coursefullname, 1);
    }
     ...
     * @return void
     */
    public function format_muonline_tile_has_colour(int $tilenumber, string $expectedvalue) {
        ...
        $jsscript = "(() => {return document.querySelector('style#format-muonline-dynamic-css') !== null;})();";
     ...
     * @return void
     */
    public function format_muonline_tile_has_icon(int $tilenumber, string $icon) {
     ...
     * @Given /^Muonline JS config element exists on page$/
     * @return void
     */
    public function format_muonline_js_config_exists_on_page() {
        $script = "(() => {return document.getElementById('format-muonline-js-config') !== null;})()";
        $result = $this->getSession()->evaluateScript($script);
        if (!$result) {
            throw new \Behat\Mink\Exception\ExpectationException(
                "Muonline JS config div not found",
                $this->getSession()
            );
        }
    }
    ...
    /**
     * * Set course format option for submuonline off for course.
     *
     * @Given /^format_muonline submuonline are off for course "(?P<coursefullname_string>(?:[^"]|\\")*)"$/
     * @param string $coursefullname
     * @throws dml_exception
     */
    public function format_muonline_sub_muonline_are_off_for_course($coursefullname) {
        $this->sub_muonline_on_off($coursefullname, 0);
    }

    /**
     * Set course format option for submuonline on or off for course.
     * @param string $coursefullname
     * @param int $onoff
     * @throws dml_exception
     */
    private function sub_muonline_on_off($coursefullname, $onoff) {
        global $DB;
        $onoff = $onoff ? 1 : 0;
        $courseid = $DB->get_field('course', 'id', ['fullname' => $coursefullname], MUST_EXIST);
        $courseformat = course_get_format($courseid);
        $courseformat->update_course_format_options(['id' => $courseid, 'courseusesubmuonline' => $onoff]);
    }
     ...
     *
     * @Given /^format_muonline progress indicator is showing as "(?P<progresstype_string>(?:[^"]|\\")*)" for course "(?P<coursefullname_string>(?:[^"]|\\")*)"$/
     ...
     *
     * @Then /^format_muonline progress for "(?P<modtype_string>(?:[^"]|\\")*)" called "(?P<activitytitle_string>(?:[^"]|\\")*)" in "(?P<coursefullname_string>(?:[^"]|\\")*)" is "(?P<value>\d+)" in the database$/
     ...
     *
     * @Then /^activity in format muonline is dimmed "(?P<activityname_string>(?:[^"]|\\")*)"$/
     * @param string $activityname
     * @return bool
     */
    public function activity_in_format_muonline_is_dimmed($activityname) {
     ...
     *
     * @Then /^activity in format muonline is not dimmed "(?P<activityname_string>(?:[^"]|\\")*)"$/
     * @param string $activityname
     * @return bool
     */
    public function activity_in_format_muonline_is_not_dimmed($activityname) {
        return !$this->activity_in_format_muonline_is_dimmed($activityname);
    }
    ...
    public function wait_until_activity_exists_in_format($activitytitle, $format) {
        // @codingStandardsIgnoreEnd.
        if ($format == 'subtile' || $format == 'submuonline') {
            $liclass = 'subtile';
     ...
     *
     * @Given /^I click format muonline activity "(?P<activityname_string>(?:[^"]|\\")*)"$/
     * @param string $activityname
     * @throws Exception
     */
    public function click_format_muonline_activity($activityname) {
     ...
     *
     * @Given /^I click format muonline progress indicator for "(?P<activitytitle_string>(?:[^"]|\\")*)"$/
     * @param string $activitytitle
     * @throws Exception
     */
     ...
     *
     * @Given /^I click format muonline subtile progress indicator for "(?P<activitytitle_string>(?:[^"]|\\")*)"$/
     * @param string $activitytitle
     * @throws Exception
     */
    public function i_click_subtile_progress_indicator_for($activitytitle) {
        $selector = "button[data-action=muonline-toggle-manual-completion-subtile][data-activityname='{$activitytitle}']";
        $this->execute("behat_general::i_click_on", [$selector, "css_element"]);
        $this->execute('behat_general::wait_until_the_page_is_ready');
        $this->wait_for_pending_js();  // Important to wait for pending JS here so as await AJAX response.
    }
     ...
     *
     * @Given /^format_muonline progress indicator for tile "(?P<tilenumber>\d+)" is "(?P<numcomplete>\d+)" out of "(?P<outof>\d+)"$/
     * @param string $tilenumber
    ...
    private function i_show_hide($sectionnumber, $showhide, $xpath) {
        ...
        $strhide = get_string($showhide, 'format_muonline');
        $this->execute(
     ...
     *
     * @Given /^I log out muonline$/
     */
    public function i_log_out_muonline() {
    ...
    public function tile_should_show_photo($coursename, $sectionnumber, $photoname) {
        ...
        $tilephoto = new \format_muonline\local\tile_photo($context, $sectionid);
        ...
        $imageurl = $CFG->wwwroot . "/pluginfile.php/" . $context->id
            . '/format_muonline/tilephoto/' . $sectionid . '/tilephoto/' . $photoname;

        $muonlinetyle = get_config('format_muonline', 'muonlinetyle');
        $xpath = in_array($muonlinetyle, ["1", "2"])
        ...
        if (!$nodestyle || strpos($nodestyle, $imagebaseurl) === false || strpos($nodestyle, $imageextension) === false) {
            throw new \Behat\Mink\Exception\ExpectationException(
                ...
                . "in style string '$nodestyle' for tile style '$muonlinetyle'",
                $this->getSession()
            );
        }
        return true;
    ...
    public function tile_should_show_no_photo($coursename, $sectionnumber) {
        ...
        $photo = \format_muonline\local\format_option::get(
            $courseid,
            format_muonline\local\format_option::OPTION_SECTION_PHOTO,
            $sectionid
        );
        if ($photo) {
            throw new \Behat\Mink\Exception\ExpectationException(
    ...
    public function should_see_section_confirm_delete($sectionname) {
        // @codingStandardsIgnoreEnd.
        $moodlerelease = \format_muonline\local\util::get_moodle_release();
     ...
     * (To avoid having to have multiple Muonline plugin versions).
     *
