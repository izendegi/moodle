@format @format_muonline @format_muonline_mod_modal @format_muonline_pdf_modal_student  @javascript @_file_upload
Feature: PDFs can be set to open in modal windows
  ...
  Background:
    ...
    And the following "courses" exist:
      ...
      | Course 1 | C1-pdf    | muonline  | 0             | 5           | 1                |
    And the following "activities" exist:
      ...
      | resource | Test PDF       | File description      | C1-pdf | pdf1     | 1       | 1       | 1          | course/format/muonline/tests/fixtures/test.pdf | 1        |
    ...
    # First with submuonline off as student
  @javascript
  Scenario: Open modal PDF as student with submuonline off
    When format_muonline submuonline are off for course "Course 1"
    ...
    And I wait until the page is ready
    And I click format muonline activity "Test PDF"
    And I wait until the page is ready
    And "Test PDF" "dialogue" should be visible

    And format_muonline progress for "resource" called "Test PDF" in "Course 1" is "0" in the database
    And I click on "Mark as done" "button" in the "Test PDF" "dialogue"
    And I wait until the page is ready
    And format_muonline progress for "resource" called "Test PDF" in "Course 1" is "1" in the database
    ...
    # First with submuonline off as teacher
  @javascript
  Scenario: Open modal PDF as student with submuonline off
    When format_muonline submuonline are off for course "Course 1"
    And I log in as "teacher1"
    ...
    And I log out muonline
      ...
      # Now the same again with submuonline on
  @javascript
  Scenario: Open modal PDF as student with submuonline on
    When format_muonline submuonline are on for course "Course 1"
    And I log in as "student1"
    And I am on "Course 1" course homepage
    And I wait until the page is ready
    And I click on tile "1"
    ...
    And "Test PDF" "dialogue" should not be visible
    And I click on close button for tile "1"
    And Muonline JS config element exists on page
    And I log out muonline

  # Now the same again for student with submuonline on
  @javascript
  Scenario: Open modal PDF as student with submuonline on
    When format_muonline submuonline are on for course "Course 1"
    And I log in as "teacher1"
