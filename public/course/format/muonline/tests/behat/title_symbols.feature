@format @format_muonline @format_muonline_title_symbols @javascript
Feature: When tile and activity titles are shown, symbols like "&" are displayed correctly
  ...
  @javascript
  Scenario: Activity headings are shown correctly with "&" symbol for submuonline
  ...
  @javascript
  Scenario: Activity headings are shown correctly with "&" symbol for non-submuonline
    When I log in as "student1"
  ...
  Scenario: Page modal content shown correctly with "&" symbol
    ...
    And I click on tile "1"
    And I wait until the page is ready
    And I click format muonline activity "Page with & symbol"
    And I wait until the page is ready
