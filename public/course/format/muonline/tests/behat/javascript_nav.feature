@format @format_muonline @format_muonline_javascript_nav @javascript
Feature: Interface can be enhabced with JS nav if allowed by site admin
  ...
  Scenario: Open section 1 then close and open section 2 with JS as student
    ...
    #  We check for activities in the #format-muonline-multi-section-page element not the page, as course index contains them too.
    And I should not see "Test book name s2" in the "#format-muonline-multi-section-page" "css_element"
    And "#editsectiontbtn-1" "css_element" should not be visible
    And I click on close button for tile "1"
    And I should not see "Test book name s1" in the "#format-muonline-multi-section-page" "css_element"
    And section "1" should be visible
    And section "2" should be visible
    And section "3" should be visible
    ...
    And I wait until the page is ready
    And I should not see "Test book name s1" in the "#format-muonline-multi-section-page" "css_element"
    And I should see "Test book name s2" in the "#format-muonline-multi-section-page" "css_element"
  ...
  Scenario: Open section 1 then close and open section 2 without JS as student
    When the following config values are set as admin:
      | config           | value | plugin       |
      | usejavascriptnav | 0     | format_muonline |
    ...
    And I wait until the page is ready
    And I should see "Test book name s1"
    And I should not see "Test book name s2" in the "#single_section_muonline" "css_element"
    ...
    And I click on tile "2"
    And I wait until the page is ready
    And I should not see "Test book name s1" in the "#single_section_muonline" "css_element"
    And I should see "Test book name s2" in the "#single_section_muonline" "css_element"

    And I am on "Course 1" course homepage
    And I click on tile "3"
    And I wait until the page is ready
    And I should not see "Test book name s2" in the "#single_section_muonline" "css_element"
    And I should not see "Test book name hidden" in the "#single_section_muonline" "css_element"
