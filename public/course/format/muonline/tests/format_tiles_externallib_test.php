...
/**
 * External muonline format functions unit tests.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class format_muonline_externallib_test extends \externallib_advanced_testcase {
    /**
     ...
     * @covers \format_muonline\external\external::set_image
    ...
    public function test_set_image(): void {
        ...
        $course = self::getDataGenerator()->create_course($this->muonlinecourseformatoptions);
        ...
        $result = \format_muonline\external\external::set_image(
            $course->id,
            $sectionid,
            $newicon,
            'tileicon',
        ...
        $result = \core_external\external_api::clean_returnvalue(\format_muonline\external\external::set_image_returns(), $result);
        // Check for 0 warnings.
        $this->assertTrue($result['status']);

        // Check it worked and data made it to db.
        $dbvalue = \format_muonline\local\format_option::get(
            $course->id,
        ...
        $this->expectException('required_capability_exception');
        $result = \format_muonline\external\external::set_image($course->id, $sectionid, $newicon, 'tileicon');
        $result = \core_external\external_api::clean_returnvalue(\format_muonline\external\external::set_image_returns(), $result);
     ...
     * @covers \format_muonline\external\external::set_session_width
     * @throws \dml_exception
     * @throws \invalid_response_exception
     * @throws \moodle_exception
     */
    public function test_set_session_width(): void {
        ...
        $result = \format_muonline\external\external::set_session_width($course->id, 1366);
        $result = \core_external\external_api::clean_returnvalue(
            \format_muonline\external\external::set_session_width_returns(),
            $result
        );
        // Check for 0 warnings.
        $this->assertTrue($result['status']);
        ...
        $sessionvarname = 'format_muonline_width_' . $course->id;
