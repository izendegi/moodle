...
function helper_restore_test_course(string $filename, string $newcoursename): int {
    ...
    $mbzpath = "$CFG->dirroot/course/format/muonline/tests/fixtures/mbz/$filename";
    ...
    $filerecord->component = 'format_muonline';
    ...
    $fs->delete_area_files($ctx->id, 'format_muonline', 'coursebackups');
...
function helper_set_legacy_tilephoto(int $courseid, int $sectionnumber, string $filename) {
    ...
    $filepath = "$CFG->dirroot/course/format/muonline/tests/fixtures/images";
    $sectionid = $DB->get_field(
        'course_sections',
        'id',
        ['course' => $courseid, 'section' => $sectionnumber]
