...
$pageurl = new moodle_url('/course/format/muonline/editor/migratecoursedata.php', $pageurlparams);
$PAGE->set_url($pageurl);
$PAGE->set_context($pagecontext);

if ($courseid) {
    $courseurl = new moodle_url('/course/view.php', ['id' => $courseid]);
    $course = $DB->get_record('course', ['id' => $courseid, 'format' => 'muonline']);
    if (!$course) {
        debugging("Course $courseid is not found or not a muonline course");
    ...
    $countlegacyoptions = $DB->get_field_sql(
        "SELECT COUNT(cfo.id) FROM {course_format_options} cfo
                WHERE cfo.courseid = ? AND  cfo.format = 'muonline'
    ...
    if ($sesskey) {
        require_sesskey();
        \format_muonline\local\format_option::migrate_legacy_format_options($courseid);
        \core\notification::success(
            get_string('migratedcourseid', 'format_muonline', $courseid)
        ...
        echo html_writer::tag(
            'h4',
            get_string('suremigratelegacyoptions', 'format_muonline', $countlegacyoptions),
            ['class' => 'mb-3']
        );
...
$settingsurl = new moodle_url('/admin/settings.php', ['section' => 'formatsettingmuonline']);
$PAGE->set_heading(get_string('admintools', 'format_muonline'));
...
$PAGE->navbar->add(get_string('pluginname', 'format_muonline'), $settingsurl);
$PAGE->navbar->add(get_string('migratecoursedata', 'format_muonline'));

$legacycourses = $DB->get_records_sql(
    "SELECT * FROM
        (SELECT c.id as courseid, c.fullname,
            (SELECT COUNT(cfo.id) FROM {course_format_options} cfo
                WHERE cfo.courseid = c.id AND  cfo.format = 'muonline' AND cfo.name IN('tilephoto', 'tileicon')) as legacyoptions,
            (SELECT COUNT(tfo.id) FROM {format_muonline_tile_options} tfo
                WHERE tfo.courseid = c.id AND tfo.optiontype IN (?, ?)) as newoptions
        FROM {course} c
         ) counts
    WHERE counts.legacyoptions > 0",
    [\format_muonline\local\format_option::OPTION_SECTION_PHOTO, \format_muonline\local\format_option::OPTION_SECTION_ICON]
);
$table = new html_table();
$table->head = [
    get_string('course'),
    get_string('legacytiledata', 'format_muonline'),
    get_string('newtiledata', 'format_muonline'),
    get_string('migratenow', 'format_muonline'),
];
$table->data = [];
foreach ($legacycourses as $legacycourse) {
    $table->data[] = [
        ...
        html_writer::link(
            new moodle_url(
                '/course/format/muonline/editor/migratecoursedata.php',
...
echo html_writer::div(get_string('unmigratedcoursesintro', 'format_muonline', count($legacycourses)), 'mb-2');
