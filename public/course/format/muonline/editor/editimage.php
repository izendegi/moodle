...
use format_muonline\form\upload_image_form;
...
if (!get_config('format_muonline', 'allowphotomuonline')) {
    throw new moodle_exception('disabledbyadmin', 'format_muonline');
}

$pageargs = ['sectionid' => $sectionid];
$sectionname = get_section_name($courseid, $section->section);
$url = new moodle_url('/course/format/muonline/editor/editimage.php', $pageargs);
$PAGE->set_url($url);
...
$PAGE->navbar->add(get_string('uploadnewphoto', 'format_muonline'));

$tilephoto = new \format_muonline\local\tile_photo($coursecontext, $sectionid);

if ($deletephoto) {
    require_sesskey();
    $tilephoto->clear();
    // Delete any other stored photo files for this tile.
    \format_muonline\local\tile_photo::delete_files_from_ids($courseid, $sectionid);
    \core\notification::success(get_string('imagedeletedfrom', 'format_muonline', $sectionname));
...
if ($mform->is_cancelled()) {
    ...
    if ($newfilename = $mform->get_new_filename('tileimagefile')) {
        ...
        if (!\format_muonline\local\tile_photo::verify_file_type($tempfile)) {
            ...
            try {
                ...
                if ($verifyaspectratio['status'] !== true) {
                    ...
                    \core\notification::success(
                        get_string('imagesavedfor', 'format_muonline', "'" . $sectionname . "'")
                    );
                }
                $tempfile->delete();
            } catch (Exception $e) {
...
$PAGE->requires->js_call_amd('format_muonline/edit_upload_image_helper', 'init');
echo $OUTPUT->header();
