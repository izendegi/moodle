...
use format_muonline\local\course_section_manager;
...
$pageurl = new moodle_url('/course/format/muonline/editor/admintools.php', ['action' => $action]);
...
$PAGE->navbar->add(get_string('admintools', 'format_muonline'));
...
function permitted_colours() {
    ...
    $records = $DB->get_records_select(
        'config_plugins',
        "plugin = 'format_muonline' AND " . $DB->sql_like('name', '?', false),
        ["tilecolour%"]
    );
    $permittedcolours = [];
...
function reset_colours($settingsurl, $pageurl) {
    ...
    if (count($permittedcolours) === 0) {
        redirect(
            $settingsurl,
            get_string('novaliddefaultcolour', 'format_muonline'),
            null,
    ...
    if (!optional_param('sure', 0, PARAM_INT)) {
        ...
        $requiredchangecount = $DB->count_records_sql(
            "SELECT COUNT(courseid) FROM {course_format_options}
                WHERE format = 'muonline' AND name = 'basecolour'
                AND value " . $permittedcolourssql,
            $params
        );

        if ($requiredchangecount === 0) {
            redirect($settingsurl, get_string('allcoursescomplypalette', 'format_muonline'));
        } else {
            $pageurl->param('sure', '1');
            $pageurl->param('sesskey', sesskey());
            $o = html_writer::div(get_string('sureresetcolours', 'format_muonline', $requiredchangecount), 'mb-3 mt-3');
            $o .= html_writer::link($pageurl, get_string('resetcolours', 'format_muonline'), ['class' => 'btn btn-danger']);
        ...
        $defaultvalue = get_config('format_muonline', 'tilecolour1');
        ...
        $sql = "UPDATE {course_format_options} SET value = :defaultvalue
            WHERE format = 'muonline' AND name = 'basecolour' AND sectionid = '0'
            AND value " . $permittedcolourssql;
        ...
        redirect(
            $settingsurl,
            get_string('tilecolourschanged', 'format_muonline'),
            null,
...
function list_problem_courses() {
    ...
    $o = html_writer::tag(
        ...
        . ' (' . get_string('experimental', 'format_muonline') . ')'
    );
    ...
    $countlegacyoptions = $DB->get_field_sql(
        "SELECT COUNT(cfo.id)
                ...
                WHERE cfo.format = 'muonline'
                AND cfo.name IN('tilephoto', 'tileicon')"
    );

    $o .= !$countlegacyoptions ? get_string('noproblemsfound', 'format_muonline') :
        html_writer::div(
            html_writer::link(
                new moodle_url('/course/format/muonline/editor/migratecoursedata.php'),
                get_string('migratecoursedata', 'format_muonline'),
