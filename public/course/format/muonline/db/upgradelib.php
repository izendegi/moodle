...
/**
 *
 * This method finds all courses in 'muonline' format that have actual number of sections
 * bigger than their 'numsections' course format option.
 * For each such course we call {@see format_muonline_upgrade_hide_extra_sections()} and
 * either delete or hide "orphaned" sections.
 * @throws dml_exception
 * @throws coding_exception
 */
function format_muonline_upgrade_remove_numsections() {
    global $DB;
    ...
    $params = ['format1' => 'muonline', 'format2' => 'muonline', 'numsections' => 'numsections'];
    ...
    foreach ($needfixing as $courseid => $numsections) {
        format_muonline_upgrade_hide_extra_sections($courseid, $numsections);
    }

    $DB->delete_records('course_format_options', ['format' => 'muonline', 'sectionid' => 0, 'name' => 'numsections']);
}
 ...
 * @throws coding_exception
 */
function format_muonline_upgrade_hide_extra_sections($courseid, $numsections) {
    global $DB;
 ...
 * @throws dml_exception
 */
function format_muonline_remove_unused_format_options() {
    global $DB;
    $DB->delete_records('course_format_options', ['format' => 'muonline', 'name' => 'showachladderbutton']);
    $DB->delete_records('course_format_options', ['format' => 'muonline', 'name' => 'prefixtitlewithnumber']);
    $DB->delete_records('course_format_options', ['format' => 'muonline', 'name' => 'showgradesbutton']);
}
