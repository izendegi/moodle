...
/**
 * Load the format_muonline JavaScript for the course edit settings page /course/edit.php?id=xxx
 *
 * @module      format_muonline/completion
...
define(["jquery", "core/templates", "core/config", "core/ajax", "core/str", "core_course/manual_completion_toggle"],
    function ($, Templates, config, ajax, str, coreManualCompletion) {
        ...
        const Selector = {
            ...
            toggleCompletionSubtile: '[data-action="muonline-toggle-manual-completion-subtile"]',
            tileNumber: "#tile-",
        ...
        const setProgressIndicator = function(sectionNumber, numComplete, numOutOf) {
            ...
            if (tileProgressIndicator.length) {
                ...
                if (sectionNumber === 0) {
                    const overallProgressOuter = $('#muonline-overall-progress-outer');
                    if (percent === 100) {
        ...
        const triggerCompletionChangedEvent = function (sectionNum, cmId) {
            if (sectionNum > 0 || cmId > 0) {
                ...
                $(document).trigger('format-muonline-completion-changed', data);
            }
        };

        /**
         * If we have called format_muonline_get_section_information then we need to add the result to the DOM.
                ...
                if (sec.sectionnum > 0) {
                    ...
                    if (availabilityInfoDiv.length > 0 && sec.isavailable && !sec.availabilitymessage) {
                        ...
                        if (availabilityInfoDiv.length > 0) {
                            ...
                            Templates.render("format_muonline/availability_info", {
        ...
        /**
         * Sometimes we must check the availability and completion status of/some all muonline using AJAX.
         * This might happen if for example a tile expands and some embedded activities are then complete.
         * Other muonline might use the completion of a previous tile for their availability.
        ...
        return {
            init: function (courseIdInit) {
                ...
                $(document).ready(function () {
                    var loadingString = '...';
                    str.get_strings([{key: "loading", component: "format_muonline"}]).done(function (s) {
                        loadingString = s[0] + '  ...';
                    });
                    // Included like this so that later dynamically added boxes are covered.
                    // We only need to handle this here for submuonline.
