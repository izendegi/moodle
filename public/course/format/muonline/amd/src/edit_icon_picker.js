...
/**
 * Javascript Module to handle the icon picker dialogue for format_muonline
 * which the editing user uses to select an icon for a tile or the default icon
 * for all muonline in the course
 *
 * @module      format_muonline/icon_picker
    ...
    function ($, Templates, ajax, str, Notification, config) {
        ...
        var getAndStoreIconSet = function(courseId, callback) {
            var photosPromises = ajax.call([{
                methodname: "format_muonline_get_icon_set",
        ...
        var getPhotoTileButtonUrl = function(courseId, sectionId) {
            return config.wwwroot
                + '/course/format/muonline/editor/editimage.php?sectionid=' + sectionId;
        };
        ...
        var setIcon = function (
            ...
            const onCoursePage = ['section-view-muonline', "course-view-muonline", 'course-view-section-muonline'].includes(pageType);
            var changeUiTilePhoto = function (jqueryObjToChange, imageUrl, imageType) {
                ...
                switch (imageType) {
                    ...
                    case 'tilephoto':
                        if (!imageUrl) {
                            // Unsetting photo.
                            templateParams.tileicon = $('#format-muonline-js-config').data('defaultcourseicon');
                            templateToRender = 'tileicon';
                            break;
                        }
                ...
                divToAnimate.animate({opacity: 0}, 500, function () {
                    Templates.render("format_muonline/" + templateToRender, templateParams)
                        .done(function (html) {
            ...
            var setIconDbPromises = ajax.call([{
                methodname: "format_muonline_set_image",
                ...
                if (response.status === true) {
                    if (onCoursePage) {
                        ...
                        if (imageType === "tileicon") {
                            Templates.renderPix("tileicon/" + icon, "format_muonline", displayname)
         ...
         * @param {int} sectionId
         * @param {int} section
         * @param {int} allowPhotoMuonline whether to render a button for the photo tile form - true or false).
         * @param {number} maxNumberIcons
         */
        var launchIconPicker = function (pageType, courseId, sectionId, section,
                                         allowPhotoMuonline, maxNumberIcons) {
            ...
            if (typeof modalStored !== "object") {
                ...
                var renderModal = function() {
                    Templates.render("format_muonline/icon_picker_modal_body", {
                        /* eslint-disable-next-line camelcase */
                        icon_picker_icons: iconSet,
                        showphotos: allowPhotoMuonline,
                        ...
                        require(["core/modal"], function (coreModal) {
                            coreModal.create({
                                ...
                                if (allowPhotoMuonline) {
                                    ...
                                    $("#launch-photo-library").click(function () {
                                        if (recentPhotoSet.length !== 0) {
                                            Templates.render("format_muonline/icon_picker_photos", {
                                                /* eslint-disable-next-line camelcase */
                                ...
                                $(document).trigger('format-muonline-icon-picker-modal-created');
                            });
                        });
                    });
                };
        ...
        return {
            init: function (courseId, pageType, allowPhotoMuonline, maxNumberIcons) {
                $(document).ready(function () {
                    var stringKey = allowPhotoMuonline ? "picknewiconphoto" : "picknewicon";
                    str.get_string(stringKey, "format_muonline").done(function (pickAnIcon) {
                    ...
                    pageContent.on("click", '[data-muonline-action="launch-muonline-icon-picker"]', function (e) {
                        e.preventDefault();
