...
define(["jquery", "core/ajax"], function ($, ajax) {
    ...
    var Selector = {
        ...
        TILES_OUTER: ".format-muonline.jsenabled #format-muonline-multi-section-page",
        TILES: ".format-muonline.jsenabled ul.muonline",
        ACTIVITY: ".activity",
        SPACER: ".spacer",
        SECTION_ID: "#section-",
     ...
     * @see \format_muonline\local\util::width_template_data() for more information.
     * @return {Promise}
     */
    var resizeMuonlineDivWidth = function() {
        ...
        const muonline = $(Selector.TILES);
        const muonlineOuter = $(Selector.TILES_OUTER);
        const TILE_WIDTHS = {
            standard: 260,
            min: 225,
            mobileMin: 160
        ...
        try {
            var muonlineParentWidth = muonlineOuter.parent().innerWidth();
            var firstTile = $(muonline.find(Selector.TILE)[0]);
            ...
            // Skip if window is only 2 or smaller than 2 muonline wide already.
            // This ensures that we don't crush the muonline into the centre (i.e. we use at least x% of width).
            var maxPossibleMuonlinePerRow = Math.floor(muonlineParentWidth / TILE_WIDTHS.min);
            var tileCount = ($(Selector.TILE).not(Selector.SPACER).length); // How many muonline in this course.
            if (muonlineParentWidth < TILE_WIDTHS.mobileMin * 2) {
                // Only space for one tile - don't resize to save space.
                dfd.reject("Too narrow to resize");
            } else if (tileCount <= 3 && muonlineParentWidth > tileCount * TILE_WIDTHS.mobileMin) {
                resizeWidth = TILE_WIDTHS.standard * 4;
            } else if (muonlineParentWidth < TILE_WIDTHS.min * 3) {
                resizeWidth = (TILE_WIDTHS.standard - tileMargin) * 2;
            } else if (maxPossibleMuonlinePerRow < 4) {
                // Here we set the max width to the space we have available, so that muonline are centred.
                resizeWidth = TILE_WIDTHS.standard * maxPossibleMuonlinePerRow;
            } else {
                ...
                // How many muonline per row could we fit on the screen, if we put in as many as possible?
                var rowMaxCount = Math.min(
                    Math.floor(muonlineParentWidth / oneTileWidth),
                    tileCount
                );
                // If we'd have a single row of more than 5 muonline, split it into 2.
                if (tileCount <= rowMaxCount && tileCount > 5) {
                    ...
                    // If we can fit 3 muonline per row max, we don't restrict to 2, as this makes content window unnecessarily small.
                    // Also if we have 3 or more rows (3 or more) we don't bother restricting as the last row is not so noticeable.

                    // How many muonline per row do we want as a minimum (in order to occupy a reasonable amount of width)?
                    var rowMinCount = Math.floor(muonlineParentWidth / oneTileWidth);

                    // What are the possibilities for muonline per row?  Then we can look at which we want.
                    var possibleRowCounts = range(rowMinCount, rowMaxCount).reverse(); // Something like [6, 5, 4, 3].

                    // For each possibility, how many muonline would that leave on the last row?
                    var lastRowRemainderMuonline = possibleRowCounts.map(function (num) {
                        return tileCount % num;
                    });
                    if (lastRowRemainderMuonline.indexOf(0) !== -1) {
                        // We have the option of having a *full* last row so take that.
                        resizeWidth = oneTileWidth * possibleRowCounts[lastRowRemainderMuonline.indexOf(0)];
                    } else if (tileCount < rowMinCount) {
                        resizeWidth = tileCount * oneTileWidth;
                    } else {
                        // Otherwise make the last row as full as possible (few muonline on last row looks worse).
                        resizeWidth = oneTileWidth * possibleRowCounts[lastRowRemainderMuonline.indexOf(
                            Math.max.apply(null, lastRowRemainderMuonline)
                        )];
                    }
                } else {
                    // In these cases, we don't artificially narrow the view, but we do put a max width on of the existing width.
                    // This is so that, when sections open under the muonline, they do not stick out with extra width beyond the muonline.
                    // It also allows the auto margin to centre the muonline.
                    resizeWidth = rowMaxCount * oneTileWidth;
                }
            }

            // If we already have the desired width, nothing to do here so skip it.
            // We use the width on the outer element muonlineOuter so as to restrict width of section zero as well.
            var existingWidth = parseInt(muonlineOuter.css("max-width").replace("px", ""));
            if (Math.abs(resizeWidth - existingWidth) < 50) {
                ...
                muonlineOuter.css("max-width", winWidth).animate({"max-width": resizeWidth}, resizeTime, "swing",
                    function() {
                        dfd.resolve();
            ...
            timeoutBeforeResizeAjax = setTimeout(function () {
                ...
                ajax.call([{
                    methodname: "format_muonline_set_session_width",
                    args: {courseid: courseId, width: Math.floor(resizeWidth)}
                }]);
            }, 1000);
        } catch (err) {
            // Unset widths as something went wrong.
            muonlineOuter.css("max-width", winWidth).animate({"max-width": "100%"}, 500, "swing");
            ajax.call([{
                methodname: "format_muonline_set_session_width",
    ...
    var organiser = {
         ...
         * e.g. we have a row of muonline 1-3 and then after that we need to have the content divs which contain the
        ...
        getContentSectionPositions: function () {
            ...
            var maxMuonlinePerRow = 1;
            var thisRowCount = 0;
            var allMuonline = $(Selector.TILES).children(Selector.TILE).not(Selector.TILE_COLLAPSED).not(".spacer");
            if (allMuonline.length === 0) {
                // Course has no sections.
                return [];
            }
            allMuonline.each(function (index, tile) {
                ...
                if (currentSectionId) {
                    if (index === 0) {
                        ...
                        // maxVerticalPositionDifference is because muonline on same row may have different vertical positions.
                        // E.g. if one of the is in a hover state.  If they are within 100 px max they must be on same row.
                    } else {
                        // On a new row.
                        thisRowCount = 0;
                    }
                    if (thisRowCount > maxMuonlinePerRow) {
                        maxMuonlinePerRow = thisRowCount;
                    }
                    previousTile = tile;
                }
            });
            ...
            // Now allocate rows of maxMuonlinePerRow each until we run out of muonline.
            allMuonline.each(function (index, tile) {
                currentSectionId = $(tile).data("section");
                if (rows.length === 0 || rows[rows.length - 1].sections.length >= maxMuonlinePerRow) {
                    if (rows.length >= 1) {
        ...
        /**
         * Move content sections to appear under the correct muonline
            ...
            positionData.forEach(function (row) {
                row.sections.forEach(function (contentSection) {
                    if (row.displayAfterTile === positionData[positionData.length - 1].displayAfterTile) {
                        $(Selector.SECTION_ID + contentSection).detach().insertAfter($("ul.muonline .tile").last());
                    } else {
            ...
            organiser.moveContentSectionsToPlaces(
                ...
                [
                    function() {
                        ...
                        dfd.resolve("Finished organising muonline");
                        reOrgLocked = false;
                    }
                ]
            );
    ...
    /**
     * On initial page load, we need to unhide the muonline.  They will have been hidden from PHP if we are using JS.
     * This is to cover the initial setting up of div width (i.e. allow us time to get screen width and set up).
     */
    const unHideMuonline = function() {
    ...
    return {
        init: function(courseIdInit, sectionOpen, fitMuonlineToWidth, isEditing) {
            courseId = courseIdInit;
            $(document).ready(function() {
                if (!isSectionPage) {
                    // When we first load the page we want to move the tile contents divs.
                    // Put them in the correct rows according to which row of muonline they relate to.
                    // Only then do we re-open the last section the user had open.
                    var organiseAndRevealMuonline = function () {
                    ...
                    if (fitMuonlineToWidth && !isEditing) {
                        // If we have a single tile on the last row it looks odd so resize window.
                        resizeMuonlineDivWidth().done(function() {
                            organiseAndRevealMuonline();
                        }).fail(function() {
        ...
        resizeMuonlineDivWidth: function() {
            return resizeMuonlineDivWidth();
        },
        runReOrg: function () {
