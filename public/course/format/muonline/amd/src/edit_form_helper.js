 ...
 * if the user picks an invalid option it will be detected by format_muonline::edit_form_validation (lib.php)
 * but this is to help them avoid triggering that if they have JS enabled
 *
 * @module      format_muonline/edit_form_helper
...
define(["jquery", "core/notification", "core/str", "core/templates"],
    function ($, Notification, str, Templates) {
        "use strict";
        return {
            init: function (pageType, courseDefaultIcon, courseId, allowphotomuonline, activitydocsurl) {
                $(document).ready(function () {
                    $("select#id_courseshowtileprogress").change(function (e) {
                        if (e.currentTarget.value !== "0") {
                            ...
                            if (enableCompBox.val() === "0") {
                                // We are changing to show progress on muonline
                    ...
                    $("select#id_enablecompletion").change(function (e) {
                        if (e.currentTarget.value === "0") {
                            ...
                            // It follows that we must be hiding progress on muonline too.
                    ...
                    Templates.render("format_muonline/colour_picker", {
                        ...
                        // If the course is being switched in to "Muonline", body will still have old format class e.g. format-topics.
                        // This comes from core.  We want body to have format-muonline class for our colour picker CSS, so we add it.
                        var body = $("body");
                        if (!body.hasClass("format-muonline")) {
                            body.addClass("format-muonline");
                        }
                    });
                    ...
                    if (pageType === "course-edit") {
                        ...
                        Templates.render("format_muonline/icon_picker_launch_btn", {
                            initialicon: currentIcon,
                            initialname: selectedIconName,
                            sectionId: 0,
                            allowphotomuonline: false // We do not allow a photo to be set as the default at course level.
                        }).done(function (html) {
                            $(html).insertAfter(selectBox);

                            // We can hide the original select box now as users will use the button instead.
                            selectBox.hide();
                            require(["format_muonline/edit_icon_picker"], function(iconPicker) {
                    ...
                    Templates
                        .render("format_muonline/edit_form_helptext")
                        .done(function (html) {
