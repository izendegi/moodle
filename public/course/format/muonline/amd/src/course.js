...
/**
 * Main Javascript module for format_muonline for when user is *NOT* editing.
 * See course_edit for if they are editing.
 * Handles the UI changes when muonline are selected and anything else not
 * covered by the specific modules
 *
 * @module      format_muonline/course
 * @copyright   2018 David Watson {@link http://evolutioncode.uk}
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

define(["jquery", "core/templates", "core/ajax", "format_muonline/browser_storage",
        "core/notification", "core/str", "format_muonline/tile_fitter", 'core/fragment', 'core_filters/events'],
    function ($, Templates, ajax, browserStorage, Notification, str, tileFitter, Fragment, FilterEvents) {
        ...
        var ClassNames = {
            ...
            HAS_OPEN_TILE: 'format-muonline-tile-open',
            ON_TILE_CONTROL: 'on-tile-control' // Muonline may have controls on them which do not open the section when clicked.
        };
        ...
        const OVERLAY_ID = 'format_muonline_overlay';
        ...
        /**
         * When JS navigation is being used, when a user un-selects a tile, we have to un-fade other muonline
        ...
        var setCourseContentHTML = function (contentArea, html, js) {
            ...
            $(document).trigger('format-muonline-section-content-changed', {
            ...
            var expandAndScroll = function () {
                ...
                if (iframes.length > 0) {
                    ...
                    if (enableCompletion) {
                        ...
                        setTimeout(() => {
                            $(document).trigger('format-muonline-completion-changed', {
                                courseid: courseId,
         ...
         * @param {boolean} fitMuonlineToScreenWidth whether we need to resize the muonline window while muonline are closed.
         * @returns {Promise}
         */
        var reOrgSections = function (fitMuonlineToScreenWidth) {
            ...
            if (fitMuonlineToScreenWidth) {
                tileFitter.resizeMuonlineDivWidth(courseId).done(() => {
                    // Wait until resize is done to start re-org to allow muonline to settle.
                    reOrgFunc();
                });
            } else {
                reOrgFunc();
        ...
        var getSectionContentFromServer = function (courseContextId, sectionId) {
            ...
            // This gets the fragment from format_muonline_output_fragment_get_cm_list().
            return Fragment.loadFragment(
                'format_muonline', 'get_cm_list', courseContextId, {sectionid: sectionId}
            );
        };
        ...
        var populateAndExpandSection = function(courseContextId, sectionId, sectionNumber) {
            ...
            ajax.call([{
                methodname: "format_muonline_log_tile_click", args: {
        ...
        return {
            init: function (
                ...
                $(document).ready(function () {
                    ...
                    if (useSubMuonline) {
                        // We need to be able to style tooltips outside of ul.muonline element.
                        $(Selector.BODY).addClass('format-muonline-submuonline');
                    }
                    ...
                    if (anchorMatches && anchorMatches.length) {
                        ...
                        if (anchorCmId) {
                            ...
                            openTile = null;
                            ajax.call([{
                                methodname: "format_muonline_get_course_mod_info", args: {cmid: anchorCmId}
                    ...
                    if (openTile !== 0) {
                        tileFitter.init(courseId, openTile, fitMuonlineToWidth, false);
                    } else {
                        tileFitter.init(courseId, null, fitMuonlineToWidth, false);
                    }

                    // We are going to watch for changes to size of main muonline window.
                    // This allows us to call the tile fitter to re-org muonline if needed.
                    ...
                    if (useJavascriptNav) {
                        ...
                        pageContent.on(Event.CLICK, Selector.TILE_CLICKABLE, function (e) {
                            ...
                            e.preventDefault();
                            // If other muonline have loading icons, fade them out (on the tile not the content sec).
                            $(Selector.TILE_LOADING_ICON).fadeOut(300, function () {
                                $(Selector.TILE_LOADING_ICON).html();
                            });
                        ...
                        // When window is re-sized, content sections under the muonline may be in wrong place.
                        // So remove them and re-initialise them.
                        // Collapse the selected section before doing this.
                        // Otherwise the re-organisation won't work as the muonline' flow will be out when they are analysed.
                        // We use the multi_section_muonline element to capture left and right drawer opening/closing.
                        const resizeObserver = new ResizeObserver(() => {
                            ...
                            resizeTimeout = setTimeout(function() {
                                ...
                                if (resizeRequired) {
                                    ...
                                    reOrgSections(fitMuonlineToWidth);
                                }
                            }, 100);
                        });
                        ...
                        if (isMultiSectionPage) {
                            // Remove section and cmid URL params if present as we are using JS nav and showing all muonline.
                    ...
                    } else if (fitMuonlineToWidth) {
                        tileFitter.resizeMuonlineDivWidth(courseId);
                    }
                    ...
                    $(document).on('format-muonline-completion-changed', function(e, data) {
                        ...
                        ajax.call([
                            {
                                methodname: "format_muonline_get_section_information",
                                args: {
                                    courseid: courseId,
                        ...
                        .done((response) => {
                            require(["format_muonline/completion"], function (completion) {
                    ...
                    if (enableCompletion) {
                        ...
                        // This is for non-submuonline only.
                    ...
                    if (useFilterButtons) {
                        require(["format_muonline/filter_buttons"], function (filterButtons) {
                    ...
                    // If theme is displaying the .muonline_coursenav class items, show items with this class.
                    ...
                    $(".muonline_coursenav").removeClass("hidden");

                    // Render the loading icon and store its HTML globally so that we can use it where needed later.
                    Templates.render("format_muonline/loading", {}).done(function (html) {
                    ...
                    var stringKeys = [
                        {key: "sectionerrortitle", component: "format_muonline"},
                        {key: "refresh"},
                        {key: "cancel", component: "moodle"},
                        {key: "noconnectionerror", component: "format_muonline"},
                        {key: "show"},
                        {key: "hide"},
                        {key: "other", component: "format_muonline"},
                        {key: "continue"}
                    ];
                    str.get_strings(stringKeys).done(function (s) {
                        s.forEach(function(str, index) {
                            if (str) {
                                ...
                                require(["core/log"], function(log) {
                                    log.debug(`Format muonline get_strings error ${index}`);
                                    log.debug(s);
                                });
                            }
                        });
